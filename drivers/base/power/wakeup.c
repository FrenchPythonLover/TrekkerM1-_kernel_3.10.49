...
#ifdef CONFIG_QUEUE_SYS_SYNC  //add sys_sync running queue
...
#endif  //add sys_sync running queue
...
static void  __exit wakequeue_sys_sync_exit(void)
{
	destroy_workqueue(suspend_sys_sync_work_queue);
}

static int __init queue_sys_sync_init(void)
{
	int ret = 0;
...
#ifdef CONFIG_QUEUE_SYS_SYNC  //add sys_sync running queue
module_init(queue_sys_sync_init);
module_exit(wakequeue_sys_sync_exit);
#endif   //add sys_sync running queue
