...
#define QPNP_PON_XVDD_RB_INF                   BIT(0)//hmct,coin cell spare bit
#define QPNP_PON_XVDD_RB_NONE                  0x0
...
//hmct ---the phone is on, when set alarm,store alarm type
int qpnp_pon_poweron_alarm(bool poweroff_alarm)
{
	int rc;
	u8 reg_r = 0,reg_s = 0;
	struct qpnp_pon *pon = sys_reset_dev;
	...
	reg_s = poweroff_alarm ? QPNP_PON_XVDD_RB_INF : QPNP_PON_XVDD_RB_NONE;

	rc = spmi_ext_register_readl(pon->spmi->ctrl, pon->spmi->sid,
			QPNP_PON_XVDD_RB_SPARE(pon->base), &reg_r, 1);//0x88e
	if (rc) {
		dev_err(&pon->spmi->dev,
	...
	if (reg_s == reg_r)
		pr_debug("pon already set alarm type poweroff alarm(%d:%d)\n",poweroff_alarm,reg_s);
	else {
		pr_err("pon set alarm type poweroff alarm(%d:%d)\n",poweroff_alarm,reg_s);
		rc = spmi_ext_register_writel(pon->spmi->ctrl, pon->spmi->sid,
						QPNP_PON_XVDD_RB_SPARE(pon->base), &reg_s, 1);
		if (rc) {
			dev_err(&pon->spmi->dev,
				"Unable to write addr=%x, rc(%d)\n",
				QPNP_PON_XVDD_RB_SPARE(pon->base), rc);
			return rc;
		}
	}
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
    ...
    //add debug info by zhaotingfa 20150202
	printk("Powerkey input: code=%d, sts=0x%hhx\n",cfg->key_code, pon_rt_sts); 
