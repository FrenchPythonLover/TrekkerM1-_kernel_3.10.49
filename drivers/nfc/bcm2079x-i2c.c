...
static const char *nfc_name;
static const char *nfc_clk_info;
		...
		unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case BCMNFC_POWER_CTL:
			...
			if (arg != 1)
				set_client_addr(bcm2079x_dev, bcm2079x_dev->original_address);
			/**********system clk operation**********/
			if (bcm2079x_dev->use_sys_clk) {
				if(arg == 1) {
					if (bcm2079x_dev->check_clock) {
						ret = bcm2079x_clock_select(bcm2079x_dev);
						bcm2079x_dev->check_clock = false;
					}
					if (ret < 0)
						goto err_req;
				} else if (arg == 0) {
					if (!bcm2079x_dev->check_clock) {
						ret = bcm2079x_clock_deselect(bcm2079x_dev);
						bcm2079x_dev->check_clock = true;
					}
					if (ret < 0)
						goto err_req;
				}
...
{
	int ret = 0;
	printk("bcm2079x_clock_select:\n");
	if (!strcmp(bcm2079x_dev->clk_src_name, "BBCLK2")) {
		bcm2079x_dev->s_clk  =
...
#if 0
	/* Set clock rate */
	ret = clk_set_rate(bcm2079x_dev->s_clk, NFC_RF_CLK_FREQ);
	if (ret)
...
{
	int ret = -1;
	printk("bcm2079x_clock_deselect:\n");
	if (bcm2079x_dev->s_clk != NULL) {
...
static int nfc_parse_dt(struct device *dev, struct bcm2079x_platform_data *pdata)
{
	...
	if (pdata->use_sys_clk){
		...
		printk("nfc_parse_dt : clkreq gpio is %d\n",pdata->clkreq_gpio);
...
static int nfc_info_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "nfc_chip_name:%s\n",nfc_name);
	if(!strcmp(nfc_clk_info, "BBCLK2"))
		seq_printf(m, "nfc_clock_info:19200\n");
	else
		seq_printf(m, "nfc_clock_info:26000\n");
	return 0;
}

static int nfcinfo_open(struct inode *inode, struct file *file)
{
	return single_open(file, nfc_info_proc_show, NULL);
}
		...
		const struct i2c_device_id *id)
{
	...
	bcm2079x_dev->check_clock = true;
	nfc_name = platform_data->chip_name;
	nfc_clk_info = platform_data->clk_src_name;
	bcm2079x_dev->wake_gpio = platform_data->wake_gpio;
	bcm2079x_dev->irq_gpio = platform_data->irq_gpio;
	bcm2079x_dev->en_gpio = platform_data->en_gpio;
	bcm2079x_dev->client = client;
	bcm2079x_dev->use_sys_clk = platform_data->use_sys_clk;
	bcm2079x_dev->clk_src_name = platform_data->clk_src_name;
	bcm2079x_dev->clkreq_gpio = platform_data->clkreq_gpio;
	...
	**/
	...
	r = proc_create("nfcinfo", 0, NULL, &proc_nfcinfo_operations);
	if (!r)
		printk("fail to create proc/nfcinfo");
...
static int bcm2079x_remove(struct i2c_client *client)
{
	...
	if (bcm2079x_dev->use_sys_clk){
		gpio_free(bcm2079x_dev->clkreq_gpio);
	}
	kfree(bcm2079x_dev);
