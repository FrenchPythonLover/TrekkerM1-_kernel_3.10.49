...
//*************** hisense code start ****************//
#ifndef NR_GPIO_IRQS
#define NR_GPIO_IRQS 121
#endif

#define GPIO_MUX_FUNC(gpio_cfg)     (((gpio_cfg) >>  2) & 0xf)
#define GPIO_INOUT_DIR(gpio_cfg)    (((gpio_cfg) >> 9) & 0x1)
#define GPIO_PULL_DIR(gpio_cfg)    		((gpio_cfg) & 0x3)
#define GPIO_DRV_STRENGTH(gpio_cfg)    (((gpio_cfg) >> 6) & 0x7)
#define GPIO_INT_CHECK(gpio_int_cfg)   ((gpio_int_cfg) & 0x1)


/* Bits of drive-strength in the GPIO_CFG register.
*/
static char *gpio_dir[] =
{
	"IN",
	"OUT",
};
...
static char* gpio_int_en[] =
{
	"Disable",
	"Enable",
};

static char *drv_strength_value[]=
{
	"DRV_2_MA",
	"DRV_4_MA",
	"DRV_6_MA",
...
static unsigned msm_gpio_get_out_value(void __iomem * base,unsigned gpio)
{
	return (__raw_readl(TLMMV4_GP_INOUT(base,gpio)) & BIT(GPIO_OUT_BIT))>>1;
}
static unsigned __msm_gpio_get_inout(void __iomem *  base,unsigned gpio)
{
	return __raw_readl_no_log(TLMMV4_GP_INOUT(base,gpio)) & BIT(GPIO_IN_BIT);
}
...
static int gpio_status_proc_show(struct seq_file *m, void *v)
{
	...
	unsigned int fuction = 0;
	unsigned int pull = 0;
	unsigned int drv_strenth = 0;
	unsigned int int_cfg = 0;
	uint32_t config;

	seq_putc(m, '\n');
	seq_printf(m, "id    \t\t  func  \t direct \t value \t pull \t\t drv_strenth \t int_cfg\n");
	for(i = 0; i <= NR_GPIO_IRQS; i++) //NR_GPIO_IRQS
	{
		config = __raw_readl(TLMMV4_GP_CFG(gpio_register_base,i));
		int_cfg = __raw_readl(TLMMV4_GP_INTR_CFG(gpio_register_base,i));
		pull = GPIO_PULL_DIR(config);
		drv_strenth = GPIO_DRV_STRENGTH(config);
		int_cfg = GPIO_INT_CHECK(int_cfg);
		fuction = GPIO_MUX_FUNC(config);
		direct = GPIO_INOUT_DIR(config);
		//direct = __raw_readl(GPIO_IN_OUT(i));
		if(direct)
			value = msm_gpio_get_out_value(gpio_register_base,i);
		else
			value = __msm_gpio_get_inout(gpio_register_base,i);

		seq_printf(m, " GPIO_%d   \t %d \t\t %s \t\t %d\t %s\t %s\t %s\n",
						i,
						fuction,
						gpio_dir[direct],
						value,
						gpio_pull[pull],
						drv_strength_value[drv_strenth],
						gpio_int_en[int_cfg]);
	}
	seq_putc(m, '\n');
	return 0;
...
void gpio_status_sleep_show(void)
{
	...
	printk("id    \t\t  func  \t direct \t value \t pull \t\t drv_strenth \t int_cfg\n");
	for(i = 0; i < NR_GPIO_IRQS; i++) //NR_GPIO_IRQS
	{
		config = __raw_readl(TLMMV4_GP_CFG(gpio_register_base,i));
		int_cfg = __raw_readl(TLMMV4_GP_INTR_CFG(gpio_register_base,i));
...
static int __init gpio_status_proc_init(void)
{
	...
	proc_create("gpio_status", 0, NULL, &gpio_status_proc_fops);
	return 0;
}
