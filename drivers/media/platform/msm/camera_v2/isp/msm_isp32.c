...
static int msm_vfe32_check_stats_cfg(struct vfe_device *vfe_dev)
{
   ...
   height = (0x1FF&(msm_camera_io_r(vfe_dev->vfe_base + 0x704)>>10));
   if(((hoffset*2) + (64*(width+1)))==camif_width){
	if(((voffset*2)+(48*(height + 1))) == camif_height)
		ret = 1;
   }
   vfe_dev->stats_vnum = (0x3F&(msm_camera_io_r(vfe_dev->vfe_base+0x704)>>26));
   vfe_dev->stats_hnum = (0x7F&(msm_camera_io_r(vfe_dev->vfe_base+0x704)>>19));
   return ret;
}
