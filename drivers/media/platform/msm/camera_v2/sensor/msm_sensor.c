...
/********hisense added for registering productinfo---start********/
	case CFG_SENSOR_PRODUCT_INFO: {
	if (cdata->cfg.sensor_info.position == BACK_CAMERA_B)
		productinfo_register(PRODUCTINFO_MAIN_CAMERA_ID, cdata->cfg.sensor_info.sensor_name, NULL);
	else
		productinfo_register(PRODUCTINFO_FRONT_CAMERA_ID, cdata->cfg.sensor_info.sensor_name, NULL);
	break;
	}
/********hisense added for registering productinfo---end********/
	default:
		rc = -EFAULT;
		break;
	}
