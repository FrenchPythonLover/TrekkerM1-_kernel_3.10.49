	...
	void *data)
{
	...
	case MSM_CAMERA_LED_LOW:
		for (i = 0; i < fctrl->torch_num_sources; i++) {
			if (fctrl->torch_max_current[i] > 0 && cfg->torch_current[i] !=0 ) {
	...
	case MSM_CAMERA_LED_HIGH:
		for (i = 0; i < fctrl->flash_num_sources; i++) {
			if (fctrl->flash_max_current[i] > 0 && cfg->flash_current[i] !=0) {
...
/********hisense added for front flash --- start********/
...
/********hisense added for front flash --- end********/
	...
	fctrl->pinctrl_info.use_pinctrl = of_property_read_bool(of_node,
						"qcom,enable_pinctrl");
/********hisense added for parsing the power mode of flash---start********/
	fctrl->pinctrl_info.use_gpio_power = false;
	fctrl->pinctrl_info.use_gpio_power = of_property_read_bool(of_node,
						"qcom,enable_gpio_power_en");
	if (fctrl->pinctrl_info.use_gpio_power == true) {
		count = of_property_count_strings(of_node, "qcom,cam-vreg-name");
		CDBG("%s qcom,cam-vreg-name count %d\n", __func__, count);
		if (count) {
			flashdata->cam_vreg = kzalloc(sizeof(struct camera_vreg_t),
				GFP_KERNEL);
			if (!flashdata->cam_vreg) {
				pr_err("%s failed %d\n", __func__, __LINE__);
				return -ENOMEM;
			}
			flashdata->num_vreg = 1;
			rc = of_property_read_string_index(of_node,
				"qcom,cam-vreg-name", 0,
				&flashdata->cam_vreg[0].reg_name);
			CDBG("%s reg_name[0] = %s\n", __func__,
				flashdata->cam_vreg[0].reg_name);
			if (rc < 0) {
				pr_err("%s failed %d\n", __func__, __LINE__);
				flashdata->num_vreg = 0;
				kfree(fctrl->flashdata);
				return rc;
			}
		}
	}
/********hisense added for parsing the power mode of flash---end********/
	if (of_get_property(of_node, "qcom,flash-source", &count)) {
		...
		pr_err("[william]---%s Handle LED Flash Ctrl by GPIO %d\n", __func__, __LINE__);
		...
		/* Read the flash-max-current for an LED if present */
		if (of_get_property(of_node, "qcom,flash-max-current", &count)) {
			count /= sizeof(uint32_t);
		...
		/* Read the flash-op-current for an LED if present */
		if (of_get_property(of_node, "qcom,flash-op-current", &count)) {
			count /= sizeof(uint32_t);
			...
			rc = of_property_read_u32_array(of_node,
				"qcom,flash-op-current",
				fctrl->flash_op_current, count);
			if (rc < 0) {
				pr_err("%s failed %d\n", __func__, __LINE__);
				goto ERROR8;
			}
			...
			for (; count < MAX_LED_TRIGGERS; count++)
				fctrl->flash_op_current[count] = 0;
		}


		/* Read the torch-max-current for an LED if present */
		if (of_get_property(of_node, "qcom,torch-max-current", &count)) {
			count /= sizeof(uint32_t);
			...
			for (; count < MAX_LED_TRIGGERS; count++)
				fctrl->torch_max_current[count] = 0;
		}

		/* Read the torch-op-current for an LED if present */
		if (of_get_property(of_node, "qcom,torch-op-current", &count)) {
			count /= sizeof(uint32_t);
			...
			rc = of_property_read_u32_array(of_node,
				"qcom,torch-op-current",
				fctrl->torch_op_current, count);
			if (rc < 0) {
				pr_err("%s failed %d\n", __func__, __LINE__);
				goto ERROR8;
			}
			...
			for (; count < MAX_LED_TRIGGERS; count++)
				fctrl->torch_op_current[count] = 0;
		}
		...
		const struct i2c_device_id *id)
{
	...
	snprintf(fctrl->msm_sd.sd.name, sizeof(fctrl->msm_sd.sd.name),
		"%s", id->name);
/********hisense added for making sure the initial status of flash is correct---start********/
	fctrl->led_state = MSM_CAMERA_LED_RELEASE;
/********hisense added for making sure the initial status of flash is correct---end********/
