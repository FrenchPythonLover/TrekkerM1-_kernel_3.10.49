...
/********hisense added for controling the flashlight node---start********/
	int32_t (*flash_led_init_flashlight)(struct msm_led_flash_ctrl_t *);
	int32_t (*flash_led_release_flashlight)(struct msm_led_flash_ctrl_t *);
/********hisense added for controling the flashlight node---end********/
};
...
/********hisense added for read the status of flash*********/
    struct msm_camera_i2c_reg_setting *status_reg;
};
...
struct msm_led_flash_ctrl_t {
	...
	uint32_t flash_duty_cycle[MAX_LED_TRIGGERS];//hisense added for flash duty cycle control
	/* Torch */
	...
	uint32_t torch_duty_cycle[MAX_LED_TRIGGERS];//hisense added for torch duty cycle control
	...
	bool front_flash_node;//hisense added for front flash node
	
/********hisense added for PWM control---start********/
	struct msm_cam_clk_info *fl_en_clk_info;
	uint16_t fl_en_clk_info_size;
	void *data_fl_en[10];

	struct msm_cam_clk_info *tor_en_clk_info;
	uint16_t tor_en_clk_info_size;
	void *data_tor_en[10];
/********hisense added for PWM control---end********/

/********hisense added for flash control---start********/
	struct led_classdev led_cl_dev;
	struct regulator *reg_flash;
/********hisense added for flash control---end********/
};
