...
extern int g_eeprom_reading_num;
#endif
...
/********hisense added for eeprom use workqueue---start*******/ 
#if defined(CONFIG_HIS_MSM_EEPROM_WORKQUEUE_INIT)
            pr_err("%s: g_eeprom_reading_num = %d\n",__func__,g_eeprom_reading_num);
            while(0 != g_eeprom_reading_num){
                pr_err("%s: wait for eeprom init over 7ms \n",__func__);
                msleep(7);
            }
            
            pr_err("%s: eeprom init over ok\n",__func__);
#endif
/********hisense added for eeprom use workqueue---end*******/ 
	...
	//compatible with sp2509 and sp2509v
	if(strncmp(slave_info->sensor_name, "sp2509_hmct_zq", 14) == 0)
	{
		uint32_t                             uiState = -1;
		int32_t                             SensorID = 911 + 9;
		gpio_request(SensorID, "SENSOR_ID");
		//gpio_direction_output(SensorID, 1);
		//gpio_set_value_cansleep(911 + 9, 1);
		pr_err("uiState %d", uiState);
		uiState = gpio_get_value_cansleep(SensorID);
		pr_err("uiState %d", uiState);
		gpio_free(SensorID);

		if(1 == uiState)
		{
			pr_err("not sp2509, probe failed");
