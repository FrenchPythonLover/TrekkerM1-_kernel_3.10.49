...
#endif
/********hisense added for eeprom use workqueue---start*******/ 
#if defined(CONFIG_HIS_MSM_EEPROM_WORKQUEUE_INIT)
int  g_eeprom_reading_num = 0;
#endif
/********hisense added for eeprom use workqueue---end*******/ 
/**
...
static int custom_hynix_define_otp_read(struct msm_eeprom_ctrl_t *e_ctrl,
	...
	uint32_t addr = 0;
	int rc =0;
	CDBG("custom_hynix_define_otp_read---start\n");
	...
	for (m = 0; m < sizeof(init_otp_array) / sizeof(init_otp_array[0]); m++){
		...
		if (rc < 0) {
			pr_err("%s: hi842 to otp mode  failed\n", __func__);
			return rc;
		}
	}
	...
	for (addr = emap->mem.addr, k = 0; k < (emap->mem.valid_size); addr++, k++) {
		...
		rc |= e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x102, 0x01, MSM_CAMERA_I2C_BYTE_DATA);
		...
		//pr_err("hi842 custom:addr:[0x%04x] value: (0x%x)\n", addr, *memptr);
		memptr++;  // must
		if (rc < 0) {
			pr_err("%s: hi842 read failed\n", __func__);
			return rc;
		}
	}
	...
	CDBG("custom_hynix_define_otp_read---end\n");

	return rc;
}

static int custom_hi553_define_otp_read(struct msm_eeprom_ctrl_t *e_ctrl,
		struct msm_eeprom_memory_map_t *emap, uint8_t *memptr) {
	int m = 0;
	int k = 0;
	uint32_t addr = 0;
	int rc =0;
	CDBG("custom_hi553_define_otp_read---start\n");

	e_ctrl->i2c_client.addr_type = MSM_CAMERA_I2C_WORD_ADDR;
	//initial  sensor

	for (m = 0; m < sizeof(hi553_init_reg_array) / (sizeof(hi553_init_reg_array[0])); m++){
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
				hi553_init_reg_array[m].reg_addr, hi553_init_reg_array[m].reg_data, 
				MSM_CAMERA_I2C_WORD_DATA);
		if (rc < 0) {
			pr_err("%s: hi553 init  failed rc %d, m %d\n", __func__, rc, m);
			return rc;
		}
	}

#if 0
	for (m = 0; m < sizeof(hi553_init_reg_array0) / (sizeof(hi553_init_reg_array0[0])); m++){
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
				hi553_init_reg_array0[m].reg_addr, hi553_init_reg_array0[m].reg_data, MSM_CAMERA_I2C_WORD_DATA);
		if (rc < 0) {
			pr_err("%s: hi553 init  failed\n", __func__);
			return rc;
		}
	}
	for (m = 0; m < sizeof(hi553_init_reg_array1) / (sizeof(hi553_init_reg_array1[0])); m++){
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
				hi553_init_reg_array1[m].reg_addr, hi553_init_reg_array1[m].reg_data, 
				MSM_CAMERA_I2C_WORD_DATA);
		if (rc < 0) {
			pr_err("*******%s: hi553 init  failed\n", __func__);
			return rc;
	...
	// set to otp mode
	for (m = 0; m < sizeof(hi553_otp_array) / sizeof(hi553_otp_array[0]); m++){
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
				hi553_otp_array[m].reg_addr, hi553_otp_array[m].reg_data, 
				MSM_CAMERA_I2C_BYTE_DATA);
		mdelay(hi553_otp_array[m].delay);
		if (rc < 0) {
			pr_err("%s: hi553 to otp mode  failed\n", __func__);
			return rc;
		}
	}
	...
	for (addr = emap->mem.addr, k = 0; k < (emap->mem.valid_size); addr++, k++) {
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 
             0x10A, (addr>>8)&0xff, MSM_CAMERA_I2C_BYTE_DATA);
		rc |= e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 
		      0x10B, addr & 0xff, MSM_CAMERA_I2C_BYTE_DATA);
		rc |= e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 
		      0x102, 0x01, MSM_CAMERA_I2C_BYTE_DATA);
		rc |= e_ctrl->i2c_client.addr_type = MSM_CAMERA_I2C_WORD_DATA;
		rc |= e_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(&(e_ctrl->i2c_client), 
		0x108, memptr, 1);
		//pr_err("hi553 custom:addr:[0x%04x] value: (0x%x)\n", addr, *memptr);
		memptr++;  // must
		if (rc < 0) {
			pr_err("%s: hi553 read failed\n", __func__);
			return rc;
		}
	}

	//set  to normol mode
	for (m = 0; m < sizeof(hi553_otp_to_norm_mode_array)/sizeof(hi553_otp_to_norm_mode_array[0]); m++){
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
				hi553_otp_to_norm_mode_array[m].reg_addr, hi553_otp_to_norm_mode_array[m].reg_data, 1);
		if (rc < 0) {
			pr_err("%s: to normal  failed\n", __func__);
			return rc;
		}
	}
	CDBG("custom_hi553_define_otp_read---end\n");

	return rc;
}


static int custom_hi846_define_otp_read(struct msm_eeprom_ctrl_t *e_ctrl,
		struct msm_eeprom_memory_map_t *emap, uint8_t *memptr) {
	int m = 0;
	int k = 0;
	uint32_t addr = 0;
	int rc =0;
	CDBG("custom_hi846_define_otp_read---start\n");

	e_ctrl->i2c_client.addr_type = MSM_CAMERA_I2C_WORD_ADDR;
	//initial  sensor

	for (m = 0; m < sizeof(hi846_init_reg_array) / (sizeof(hi846_init_reg_array[0])); m++){
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
				hi846_init_reg_array[m].reg_addr, hi846_init_reg_array[m].reg_data,
				MSM_CAMERA_I2C_WORD_DATA);
		if (rc < 0) {
			pr_err("%s: hi846 init  failed rc %d, m %d\n", __func__, rc, m);
			return rc;
		}
	}

	// set to otp mode
	for (m = 0; m < sizeof(hi846_otp_array) / sizeof(hi846_otp_array[0]); m++){
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
				hi846_otp_array[m].reg_addr, hi846_otp_array[m].reg_data,
				MSM_CAMERA_I2C_BYTE_DATA);
		mdelay(hi846_otp_array[m].delay);
		if (rc < 0) {
			pr_err("%s: hi846 to otp mode  failed\n", __func__);
			return rc;
		}
	}
	...
	for (addr = emap->mem.addr, k = 0; k < (emap->mem.valid_size); addr++, k++) {
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
             0x70A, (addr>>8)&0xff, MSM_CAMERA_I2C_BYTE_DATA);
		rc |= e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
		      0x70B, addr & 0xff, MSM_CAMERA_I2C_BYTE_DATA);
		rc |= e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
		      0x702, 0x01, MSM_CAMERA_I2C_BYTE_DATA);
		rc |= e_ctrl->i2c_client.addr_type = MSM_CAMERA_I2C_WORD_DATA;
		rc |= e_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(&(e_ctrl->i2c_client),
		0x708, memptr, 1);
		//pr_err("hi846 custom:addr:[0x%04x] value: (0x%x)\n", addr, *memptr);
		memptr++;  // must
		if (rc < 0) {
			pr_err("%s: hi846 read failed\n", __func__);
			return rc;
		}
	}

	//set  to normol mode
	for (m = 0; m < sizeof(hi846_otp_to_norm_mode_array)/sizeof(hi846_otp_to_norm_mode_array[0]); m++){
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client),
				hi846_otp_to_norm_mode_array[m].reg_addr, hi846_otp_to_norm_mode_array[m].reg_data, 1);
		if (rc < 0) {
			pr_err("%s: to normal  failed\n", __func__);
			return rc;
		}
	}
	CDBG("custom_hi846_define_otp_read---end\n");
			      ...
			      struct msm_eeprom_memory_block_t *block)
{
	...
	for (j = 0; j < block->num_map; j++) {
		...
		if (emap[j].mem.valid_size) {
			if (strncmp(eb_info->eeprom_name, "hi842",5) == 0) {
			...
			} else if (strncmp(eb_info->eeprom_name, "hi553",5) == 0) {
				rc = custom_hi553_define_otp_read(e_ctrl, &emap[j], memptr);
				if (rc < 0) {
					pr_err("%s: read failed\n", __func__);
					return rc;
				}
				//memptr += emap[j].mem.valid_size;
			} else if (strncmp(eb_info->eeprom_name, "hi846", 5) == 0) {
				rc = custom_hi846_define_otp_read(e_ctrl, &emap[j], memptr);
				if (rc < 0) {
					pr_err("%s: read failed\n", __func__);
					return rc;
				}
	...
	void __user *arg)
{
	...
	rc = copy_to_user(ptr_dest, e_ctrl->cal_data.mapdata,
		cdata.cfg.read_data.num_bytes);
#if 0 // hisense removed  for  reslove once qcam-daemon died  the camera can not work because of free the eeprom data (E50T and E50M do not use this func) ---start 
...
#endif // hisense removed  for  reslove once qcam-daemon died  the camera can not work because of free the eeprom data ---end
	return rc;
}
...
/********hisense added for eeprom use workqueue---start*******/ 
#if defined(CONFIG_HIS_MSM_EEPROM_WORKQUEUE_INIT)
static void eeprom_read_work_process(struct work_struct *work)
{
	int rc = 0;
	int j = 0;
	struct msm_eeprom_ctrl_t *e_ctrl =
    ...
    pr_err("%s:E:---start",__func__);
    mutex_lock(&msm_eeprom_mutex);
    g_eeprom_reading_num ++;
			 ...
			 const struct i2c_device_id *id)
{
	int rc = 0;
/********hisense added for eeprom use workqueue---start*******/ 
    #ifndef CONFIG_HIS_MSM_EEPROM_WORKQUEUE_INIT
    int j = 0;
    #endif
/********hisense added for eeprom use workqueue---end*******/ 
	uint32_t temp;
...
/********hisense added for eeprom use workqueue---start*******/ 	
#if defined(CONFIG_HIS_MSM_EEPROM_WORKQUEUE_INIT)
    INIT_WORK(&e_ctrl->read_work, eeprom_read_work_process);
#endif 
/********hisense added for eeprom use workqueue---end*******/   
	e_ctrl->is_supported = 0;
