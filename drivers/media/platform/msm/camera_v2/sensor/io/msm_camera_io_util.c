...
//hisense added for set rate not check rate
int msm_cam_clk_enable_not_check_rate(struct device *dev, struct msm_cam_clk_info *clk_info,
		struct clk **clk_ptr, int num_clk, int enable)
{
	int i;
	int rc = 0;
	...
	if (enable) {
		for (i = 0; i < num_clk; i++) {
			...
			if (clk_info[i].clk_rate > 0) {
				...
				rc = clk_set_rate_not_check_rate(clk_ptr[i],
					clk_rate);
				if (rc < 0) {
					pr_err("%s set failed\n",
						clk_info[i].clk_name);
