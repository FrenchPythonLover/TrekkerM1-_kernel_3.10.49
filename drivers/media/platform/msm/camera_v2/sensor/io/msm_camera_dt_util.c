...
/********hisense added for PWM flash IC ---start********/
	...
	if (rc != -EINVAL) {
		...
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_TCH_EN] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_TCH_EN] = 1;
		CDBG("%s qcom,gpio-torch-en %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_TCH_EN]);
	} else {
		rc = 0;
	}	
/********hisense added for PWM flash IC ---end********/
