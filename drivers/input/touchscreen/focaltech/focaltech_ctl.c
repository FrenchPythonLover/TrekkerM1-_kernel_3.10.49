...
*
* Copyright (c) 2010-2015, Hisense. All rights reserved.
*
...
* Return: fail <0
*/
static int fts_rw_iic_drv_mywrite(struct i2c_client *client,
	u8 *buf, int length)
{
	int ret = 0;
...
static int fts_rw_iic_drv_RDWR(struct i2c_client *client, unsigned long arg)
{
	...
	for (i = 0; i < i2c_rw_queue.queuenum; i++) {
		...
		if (copy_from_user(i2c_rw_msg[i].buf,
			data_ptrs[i], i2c_rw_msg[i].length)) {
			++i;
			ret = -EFAULT;
			break;
		}
...
* Return: fail <0
*/
static long fts_rw_iic_drv_ioctl(struct file *filp,
