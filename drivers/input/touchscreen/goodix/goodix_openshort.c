...
#include "goodix_openshort.h"

//************** Customer Config Start ***********************//
// short test
#define GTP_SHORT_GND
#define GTP_VDD         28      // 3.3V
...
static s32 gtp_open_test_init(struct i2c_client *client)
{
	...
	if (ts->pdata->test_data[ts->sensor_id] == NULL){
		SET_INFO_LINE_ERR("test-data is null.\n");
		return FAIL;
	}
	ts->test_data = ts->pdata->test_data[ts->sensor_id];
	ts->gtp_test_len = ts->pdata->test_data_len[ts->sensor_id]+ GTP_ADDR_LENGTH;
	
	gtp_i2c_write(client, ts->test_data, ts->gtp_test_len + GTP_ADDR_LENGTH);
...
s32 gt9xx_open_test(struct i2c_client * client)
{
	...
	if(ret == FAIL)
	{
		SET_INFO_LINE_ERR("failed to test init...");
        ret = FAIL;
        goto open_test_exit;
    }
...
static ssize_t gtp_sysfs_shorttest_show(struct device *dev,struct device_attribute *attr, char *buf)
{
 ...
 ssize_t gtp_get_rawdatainfo(struct i2c_client* client, char* buf)
{
    ...
    if (ret == FAIL)
    {
        num_read_chars= sprintf(buf, "%s", "Parse config failed!\n");
    }else{

        num_read_chars =sprintf(buf, "RX:%d TX:%d HIGH:%d LOW:%d\n", gt9xx_sen_num, gt9xx_sc_drv_num, 0, 0);
   }
 ...
 ssize_t gtp_get_rawdata(struct i2c_client* client, char* buf)
{
    ssize_t num_read_chars = 0;   
    s32 ret = -1;
    ...
    if (NULL == rawdata_buf)
    {
        num_read_chars=  sprintf(buf, "failed to allocate memmory for rawdata buffer\n");
        goto LEAVE_RAWDATA;
    }
    ...
    //for (i = 0; i < rawdata_cnt; ++i)
    {
        ...
        if(ret <= 0)
        {
            num_read_chars = sprintf(buf, "%s", "could not get rawdata\n");
             goto LEAVE_RAWDATA;
        }
        else
        {
...
LEAVE_RAWDATA:
    ...
    GTP_DEBUG("Rawdata buffer len: %d", num_read_chars);
    //add link 2014-3-28
    ts->gtp_rawdiff_mode = 0;
    gt9_read_coor_cmd(client);
...
static ssize_t gtp_rawdata_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{    
    return gtp_get_rawdata(i2c_connect_client, buf);
}
...
static ssize_t gtp_sysfs_rawdatavalue_show(struct device *dev,struct device_attribute *attr, char *buf)
{
    char *s = buf;
    struct i2c_client* client = i2c_connect_client;
     gtp_open_test_init(client);
    s += sprintf(s,"max_limit_value : %d, min_limit_value : %d\n max_limit_key : %d,min_limit_key : %d\n uniformity_lmt : %d .\n",max_limit_value,min_limit_value,max_limit_key,min_limit_key,uniformity_lmt);
         msleep(50);
      gtp_reset_guitar(client, 20);
     msleep(100);
        gtp_send_cfg(client);
...
static ssize_t gtp_sysfs_rawdatavalue_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
}
...
static DEVICE_ATTR(rawdata, S_IRUGO|S_IWUSR, gtp_rawdata_show, gtp_sysfs_rawdata_store);
static DEVICE_ATTR(rawdatavalue, S_IRUGO|S_IWUSR, gtp_sysfs_rawdatavalue_show, gtp_sysfs_rawdatavalue_store);
