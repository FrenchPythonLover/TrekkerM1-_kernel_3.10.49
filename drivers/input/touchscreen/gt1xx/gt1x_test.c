...
int parse_config(unsigned char *config)
{
	...
	default:
		sys.driver_num = (config[REG_DRV_SEN_CNT - REG_CONFIG_BASE] & 0x1F)
			+ (config[REG_DRV_SEN_CNT + 1 - REG_CONFIG_BASE] & 0x1F) - 1;/*have viral key*/
		sys.sensor_num = (config[REG_DRV_SEN_CNT + 2 - REG_CONFIG_BASE] & 0xFF);
		sys.sc_driver_num = sys.driver_num - (config[REG_MODULE_SWITCH_DK - REG_CONFIG_BASE] & 0x01);
		sys.sc_sensor_num = sys.sensor_num;
		sys.key_number = (sys.driver_num * sys.sensor_num - sys.sc_sensor_num * sys.sc_driver_num);
...
int gt1x_get_rawdatainfo(char *buf)
{
	u8 *largebuf;
	int ret = 0;
	...
	if (largebuf == NULL)
		return MEMORY_ERR;
	ret = read_config(largebuf, GTP_CONFIG_MAX_LENGTH);

	if (ret < 0)
		num_read_chars = snprintf(buf, PAGE_SIZE, "%s", "Parse config failed!\n");
	else
		num_read_chars = snprintf(buf, PAGE_SIZE, "RX:%d TX:%d HIGH:%d LOW:%d\n",
			sys.sensor_num, sys.driver_num, 0, 0);

	return num_read_chars;
}

int gt1x_get_rawdata(char *buf)
{
	u16 *rawdata;
	...
	if (!strncmp((const char *)gt1x_version.product_id, "1143", 4))
		...
		sys.chip_type = _GT9P;
	sys.reg_rawdata_base = 0xCBB4;

	largebuf = (unsigned char *)kzalloc(2 * 1024, GFP_KERNEL);
	if (largebuf == NULL)
		return -ENOMEM;

	ret = read_config(largebuf, GTP_CONFIG_MAX_LENGTH);
	if (ret < 0) {
		GTP_ERROR("Parse config failed!");
		goto err_read_config;
	}

	r_buf = (unsigned char *)kzalloc(sys.sensor_num * sys.driver_num * 4, GFP_KERNEL);
	if (r_buf == NULL)
		goto err_alloc_rbuf;
	rawdata = (u16 *)(&r_buf[sys.sensor_num * sys.driver_num * 2]);

	ret = disable_irq_esd();
	if (ret < 0) {
		WARNING("disable irq and esd fail.");
		goto err_disable_irq_esd;
	}
	...
	if (i > 50) {
		...
		goto err_read_i2c_timeout;
	}

	ret = gt1x_i2c_read(sys.reg_rawdata_base, r_buf, sys.sensor_num * sys.driver_num * 2);
	if (ret < 0)
		goto err_read_rawdata;

	flag = 0x00;
	ret = gt1x_i2c_write(GTP_READ_COOR_ADDR, &flag, 1);
	if (ret < 0)
		goto err_write_flag;

	for (i = 0, index = 0; i < sys.sensor_num * sys.driver_num * 2; i += 2)
		rawdata[index++] = (r_buf[i] << 8) + (r_buf[i + 1] & 0x0ff);

	for (i = 0; i < sys.sensor_num * sys.driver_num; i++) {
		num_read_chars += snprintf(&(buf[num_read_chars]), PAGE_SIZE, "%u ", rawdata[i]);
		if (!((i + 1) % sys.sensor_num))
			buf[num_read_chars++] = '\n';
	}
...
err_write_flag:
err_read_rawdata:
err_read_i2c_timeout:
	be_normal();
err_enter_rawdiff_mode:
	enable_irq_esd();
err_disable_irq_esd:
	kfree(r_buf);
err_alloc_rbuf:
...
int check_version(unsigned char *ini_version)
{
	...
	snprintf((char *)tmp, PAGE_SIZE, "%d.%d.%d_%d.%d", (gt1x_version.mask_id >> 16) & 0x0FF, (gt1x_version.mask_id >> 8) & 0x0FF, (gt1x_version.mask_id & 0xFF),
...
int disable_hopping(unsigned char *config, int len, int flag)
{
	...
	if (!tmp_config) {
		GTP_ERROR("Failed to alloc memory to tmp_config.");
		return -ENOMEM;
	}
...
static int _save_short_data(u8 *short_data,
	strShortRecord r_data, u16 short_code)
{
	u8 n, cnt;
...
static int _calculate_short_resistance_with_gnd(u16 short_code, u8 pos)
{
	...
	if ((short_code & (0x8000)) == 0)	/* short to GND */
		r = (52662850) * 10 / (short_code
		& (~0x8000)) - 40 * FLOAT_AMPLIFIER * 10;	/*52662.85/code-40 */
	else			/* short to VDD */
		/* r = ( 1/(((float)(short_code&(~0x8000)))
		/0.9*0.7/1024/(sys_avdd-0.9)/40) ) -40;*/
		r = 40 * 9 * 1024 * (sys_avdd - 9) * FLOAT_AMPLIFIER
		/ (short_code & (~0x8000) * 7) - 40 * FLOAT_AMPLIFIER * 10;
	...
	if (short_code < r_th * 10) {
		...
		upload_short_data[totals * 4 + 2]
			= sys.max_driver_num + 1;	/* short with GND */
...
static int _calculate_short_resistance(strShortRecord r_data,
	unsigned short short_r_th, unsigned char flag)
{
	unsigned short j;
	...
	r = _cal_resistance(self_data[j],
		r_data.master, r_data.slave, r_data.short_code);
	...
	DEBUG("calc:short_code=%d,r_th=%d,warn=%d",
		short_code, short_r_th, warn_threshold);
	...
	if (short_code < (short_r_th * 10)) {
		...
		/* if (chk_short_data(r_data.master, r_data.slave) == 0)
			  return 0;
		*/
...
int short_test_analysis(unsigned char *result, int t_size)
{
	...
	offest += (sys.max_driver_num + sys.max_sensor_num)
	...
	if ((result != NULL) && (upload_short_data[0] != _GT9_SHORT_TEST_ERR)) {
		...
		if (warn_short_data[0] != 0) {
			if (warn_short_data[0]
				+ upload_short_data[0] > t_size)
				warn_short_data[0] =
					t_size - upload_short_data[0];

			memcpy(&result[upload_short_data[0] * 4 + 1],
				&warn_short_data[1], warn_short_data[0] * 4);
...
s32 load_code_and_check(unsigned char *codes, int size)
{
	...
	for (i = 0; i < packages; i++) {
		...
		i2c_write_data(tmp_addr,
			(u8 *) &codes[tmp_addr - start_addr], len);
...
int auto_find_ini(char *findDir, char *format, char *inipath)
{
	...
	snprintf((char *)tmp, PAGE_SIZE, "/data/test_sensor_%d.ini",
...
int ini_read(char *file, const char *key, char *value)
{
	...
	while (1) {
		...
		if (0 == strcmp((const char *)buf, (const char *)key)) {
			...
			memcpy((void *)value, (const void *)tmp,
				(size_t) strlen((const char *)tmp));
...
int ini_read_text(char *file, const char *key, unsigned char *text)
{
	...
	tmp = (char *)malloc(2 * (sys.max_sensor_num *
		sys.max_driver_num / 8) * 5 + 30);
...
static ssize_t gtp_sysfs_openshort_show(struct device *dev, struct device_attribute *attr, char *buff)
{
	...
	if (ret < 0)
		len += snprintf((char *)&buff[len], PAGE_SIZE,
			"test error code 0x%X", (unsigned int)ret);
	else if (ret == 0)
		len += snprintf((char *)&buff[len], PAGE_SIZE, "PASS");
	else {
		len += snprintf((char *)&buff[len], PAGE_SIZE, "NG\r\n");
		...
		if (ret & _BETWEEN_ACCORD_AND_LINE)
			len += (s32) snprintf((char *)&buff[len], PAGE_SIZE,
				"accord limit fuzzy\r\n");

		if (ret & _BEYOND_OFFSET_LIMIT)
			len += snprintf((char *)&buff[len], PAGE_SIZE,
				"beyond full offset limit\r\n");

		if (ret & _BEYOND_JITTER_LIMIT)
			len += snprintf((char *)&buff[len], PAGE_SIZE,
				"beyond full jitter limit\r\n");

		if (ret & _KEY_BEYOND_MAX_LIMIT)
			len += snprintf((char *)&buff[len], PAGE_SIZE,
			"beyond key max limit\r\n");

		if (ret & _KEY_BEYOND_MIN_LIMIT)
			len += snprintf((char *)&buff[len], PAGE_SIZE,
				"beyond key min limit\r\n");
		...
		if (ret & _GT_SHORT) {
			len += snprintf((char *)&buff[len], PAGE_SIZE, "short circuit\r\n");
			for (i = 0; i < result[0]; i++) {
				if (result[1 + i * 4] & 0x80) {
					len += snprintf((char *)&buff[len], PAGE_SIZE,
						"drv%d & ", result[1 + i * 4] & 0x7F);
				} else if (result[1 + i * 4]
					== (sys.max_driver_num + 1)) {
					len += snprintf((char *)&buff[len], PAGE_SIZE,
						"GND\\VDD & ");
				} else {
					len += snprintf((char *)&buff[len], PAGE_SIZE,
						"sen%d & ", result[1 + i * 4] & 0x7F);
				}
				if (result[2 + i * 4] & 0x80)
					len += snprintf((char *)&buff[len], PAGE_SIZE,
					"drv%d", result[2 + i * 4] & 0x7F);
				else if (result[2 + i * 4]
					== (sys.max_driver_num + 1))
					len += snprintf((char *)&buff[len], PAGE_SIZE,
						"GND\\VDD");
				else
...
static ssize_t gtp_sysfs_openshort_store(struct device *dev,
	struct device_attribute *attr, const char *buff, size_t count)
{
	...
	for (i = 0; i < 100; i++)
		bytes += snprintf(&buf[bytes], PAGE_SIZE, "%d,", i);
...
static DEVICE_ATTR(openshort, S_IRUGO | S_IWUSR,
	gtp_sysfs_openshort_show, gtp_sysfs_openshort_store);
