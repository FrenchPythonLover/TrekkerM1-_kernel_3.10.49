...
static ssize_t gt1x_debug_read_proc(struct file *,
	char __user *, size_t, loff_t *);
static ssize_t gt1x_debug_write_proc(struct file *,
...
static ssize_t gt1x_debug_read_proc(struct file *file,
	...
	const char *buffer, size_t count, loff_t *ppos)
{
	...
	if (strcmp(mode_str, "poweron") == 0) {
		gt1x_power_on(ts, SWITCH_ON);
		return count;
	}

	if (strcmp(mode_str, "poweroff") == 0) {
		gt1x_power_on(ts, SWITCH_OFF);
		return count;
	}
...
s32 gt1x_send_cfg(u8 *config, int cfg_len)
{
	...
	while (retry++ < 5) {
		ret = gt1x_i2c_write(GTP_REG_CONFIG_DATA,
			config, cfg_len);
		if (!ret) {
			/* at least 200ms,
			wait for storing config into flash. */
			msleep(200);
			mutex_unlock(&mutex_cfg);
...
s32 gt1x_init_panel(void)
{
	...
	const u8 *cfg_grp0 = ts->pdata->config_data[0];
	const u8 *cfg_grp1 = ts->pdata->config_data[1];
	const u8 *cfg_grp2 = ts->pdata->config_data[2];
	const u8 *cfg_grp3 = ts->pdata->config_data[3];
	const u8 *cfg_grp4 = ts->pdata->config_data[4];
	const u8 *cfg_grp5 = ts->pdata->config_data[5];
	const u8 *cfgs[] = {
		cfg_grp0, cfg_grp1, cfg_grp2,
		cfg_grp3, cfg_grp4, cfg_grp5
	};
	u8 cfg_lens[] = {
		ts->pdata->config_data_len[0],
		ts->pdata->config_data_len[1],
		ts->pdata->config_data_len[2],
		ts->pdata->config_data_len[3],
		ts->pdata->config_data_len[4],
		ts->pdata->config_data_len[5]
	};

	GTP_DEBUG("Config groups length:%d,%d,%d,%d,%d,%d",
		cfg_lens[0], cfg_lens[1], cfg_lens[2], cfg_lens[3],
		cfg_lens[4], cfg_lens[5]);

	sensor_id = gt1x_version.sensor_id;
	if (sensor_id >= 6 || cfg_lens[sensor_id]
		< GTP_CONFIG_MIN_LENGTH || cfg_lens[sensor_id]
	...
	if (cfg_len < GTP_CONFIG_MIN_LENGTH
		...
		GTP_ERROR("Config group%d is INVALID!",
			sensor_id + 1);
		return -EFAULT;
	}
	...
	if ((gt1x_abs_x_max == 0) && (gt1x_abs_y_max == 0)) {
		...
		set_reg_bit(gt1x_config[MODULE_SWITCH3_LOC],
			5, !gt1x_wakeup_level);
		gt1x_config[TRIGGER_LOC] = (gt1x_config[TRIGGER_LOC] & 0xFC) |
			gt1x_int_type;
	}
	...
	ts->config_version = gt1x_config[0];
	ret = gt1x_send_cfg(gt1x_config, gt1x_cfg_length);
	return ret;
}
	...
	while (retry--) {
		...
		if (!ret) {
			...
			if (checksum == 0 &&
				...
				GTP_ERROR
					("Read version failed!(checksum error)");
			}
		} else {
...
static s32 gt1x_wakeup_sleep(void)
{
...
#endif
#ifdef CONFIG_TOUCHSCREEN_GT1X_GESTURE
	struct goodix_ts_data *data = i2c_get_clientdata(gt1x_i2c_client);
#endif
	...
	while (retry++ < 2) {
#ifdef CONFIG_TOUCHSCREEN_GT1X_GESTURE
		if (data->gesture_wakeup_enable_pre) {
	...
	/* int  wakeup failed , try waking up by reset */
	if (ret < 0 && flag) {
		while (retry--) {
			ret = gt1x_reset_guitar();
			if (!ret)
...
void gt1x_power_reset(bool need_power_onoff)
{
	static int rst_flag;
	s32 i = 0;
	struct goodix_ts_data *ts = i2c_get_clientdata(gt1x_i2c_client);
 ...
 */
s32 gt1x_touch_event_handler(u8 *data,
	struct input_dev *dev, struct input_dev *pen_dev)
{
	...
	* 0x814E(touch status) + 8(every coordinate consist of 8 bytes data) *
	*touch num +  keycode + checksum
	*/
	if (touch_num > 1) {
...
/* start handle current event and pre-event */
	...
	if (CHK_BIT(cur_event, BIT_TOUCH_KEY) ||
		...
		for (i = 0; i < TPD_KEY_COUNT; i++) {
			if (key_value & (0x01 << i)) {
				gt1x_touch_down(tpd_virtual_key_array[i].x,
					tpd_virtual_key_array[i].y, 0, 0);
				GTP_DEBUG("Key Down.");
				break;
			}
		}
...
static void gt1x_pen_init(void)
{
	...
	input_set_abs_params(pen_dev, ABS_MT_POSITION_X, 0,
		gt1x_abs_x_max, 0, 0);
	input_set_abs_params(pen_dev, ABS_MT_POSITION_Y, 0,
		gt1x_abs_y_max, 0, 0);
...
static s32 gt1x_ps_operate(void *self, u32 command,
	void *buff_in, s32 size_in, void *buff_out,
	s32 size_out, s32 *actualout)
{
	s32 err = 0;
	s32 value;
	hwm_sensor_data *sensor_data;
...
static int gt1x_smart_cover_update_state(void)
{
	...
	if (!dev->suspended) {
		...
		#ifdef CONFIG_TOUCHSCREEN_GT1X_FH
		/* charger detector module check and send a config */
			gt1x_charger_config(1);
		#else
			ret = gt1x_send_cfg(gt1x_config, gt1x_cfg_length);
		#endif
		}
		GTP_DEBUG("Update cover state %s.",
			dev->state ? "Nearby" : "Far away");
	} else {
...
static int gt1x_smart_cover_init(void)
{
	...
	if (!sysfs_rootdir) {
		/* this kobject is shared between modules,
		do not free it when error occur */
...
#if GTP_DRIVER_SEND_CFG
	do {
		const u8 *cfg_grp0 = ts->pdata->chr_data[0];
		const u8 *cfg_grp1 = ts->pdata->chr_data[1];
		const u8 *cfg_grp2 = ts->pdata->chr_data[2];
		const u8 *cfg_grp3 = ts->pdata->chr_data[3];
		const u8 *cfg_grp4 = ts->pdata->chr_data[4];
		const u8 *cfg_grp5 = ts->pdata->chr_data[5];
		const u8 *cfgs[] = {
			cfg_grp0, cfg_grp1, cfg_grp2,
			cfg_grp3, cfg_grp4, cfg_grp5
		};
		u8 cfg_lens[] = {
		ts->pdata->chr_data_len[0],
		ts->pdata->chr_data_len[1],
		ts->pdata->chr_data_len[2],
		ts->pdata->chr_data_len[3],
		ts->pdata->chr_data_len[4],
		ts->pdata->chr_data_len[5]
	};
...
*
*/
void gt1x_charger_config(int on)
{
#if GTP_SMART_COVER
...
int gt1x_suspend(void)
{
...
#else
		ret = gt1x_i2c_read_dbl_check(GTP_REG_HN_PAIRED,
			buf, sizeof(buf));
	...
	data->gesture_wakeup_enable_pre = data->gesture_enabled;
...
int gt1x_resume(void)
{
	...
	if (!data->gesture_enabled && !gt1x_halt)
		return 0;
#else
	if (!gt1x_halt)
		return 0;
	...
	data->is_suspended = 0;
#ifdef CONFIG_TOUCHSCREEN_GT1X_FH
	if (usb_flag != data->usb_is_plugin)
		ctp_work_with_ac_usb_plugin(usb_flag);
#endif
...
s32 gt1x_init(void)
{
	...
	/* power on */
	ret = gt1x_power_init(ts, SWITCH_ON);
	if (ret < 0) {
		GTP_ERROR("power init failed.");
		return -ENODEV;
	}
	ret = gt1x_power_on(ts, SWITCH_ON);
	if (ret < 0) {
