...
struct akm_compass_data {
	...
	int	test_data[AKM_TEST_DATA_SIZE];
	...
	int hard)
{
	...
	if (hard != 0) {
		...
		if (err < 0) {
			...
			dev_err(&akm->i2c->dev, "Soft reset is done.");
		}
	}
	...
	int size)
{
	...
	if (!(AKM_DRDY_IS_HIGH(buffer[0]))) {
		dev_err(&akm->i2c->dev, "DRDY is low. Use last value.\n");
		return -EAGAIN;
	}
...
/*
	if (AKM_DOR_IS_HIGH(buffer[0]))
		dev_err(&akm->i2c->dev, "Data over run!\n");
*/
...
static int akm_self_test_set(struct sensors_classdev *sensors_cdev, int *pTest)
{
	struct akm_compass_data *akm = container_of(sensors_cdev,
			struct akm_compass_data, cdev);
	...
	if (!power_enabled) {
		...
		mode = i2c_data[0];
		dev_err(&akm->i2c->dev, "read_mode = %d.\n", mode);
	}
	...
	*pTest = (hdata[0])*(asax/128 + 1);
	*(pTest+1) = (hdata[1])*(asax/128 + 1);
	*(pTest+2) = (hdata[2])*(asax/128 + 1);
	...
	if (!power_enabled) {
		...
		if (ret < 0) {
			...
			dev_err(&akm->i2c->dev,
				"After selftest Mode is set to (%d).", mode);
			/* wait at least 100us after changing mode */
			udelay(100);
			if (AKM_IS_MAG_DATA_ENABLED() && akm->auto_report) {
				if (akm->use_hrtimer)
...
exit:
	mutex_unlock(&akm->op_mutex);
	*(pTest+3) = ((fail_total > 0) || ret) ? -EIO : 1 ;
...
int akm_compass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(s_akm->gpio_rstn)) {
		...
		err = gpio_request_one(s_akm->gpio_rstn,
				GPIOF_DIR_OUT,
				"akm09911_reset_gpio");
		if (err) {
	...
	s_akm->cdev.sensors_self_test = akm_self_test_set;
