...
#define ALS_LOW_THRESHOLD		0x0
#define ALS_HIGH_THRESHOLD		0xFFFF
...
uint32_t dynamic_intt_max_lux = 48000;
uint32_t dynamic_intt_min_lux = 0;
uint32_t dynamic_intt_min_unit = 1000;

static int als_dynamic_intt_intt[] = {EPL_ALS_INTT_2048, EPL_ALS_INTT_128};
static int als_dynamic_intt_value[] = {2048, 128};
static int als_dynamic_intt_gain[] = {EPL_GAIN_LOW, EPL_GAIN_LOW};
static int als_dynamic_intt_high_thr[] = {32000, 65535};
...
static void epl_run_ps_calibration(struct epl_sensor_priv *epl_data);
#if PS_DYN_K
...
long raw_convert_to_lux(u16 raw_data)
{
	...
	//float calculate
    dyn_intt_raw =
		(raw_data * 10) / (10 * als_dynamic_intt_value[dynamic_intt_idx] / als_dynamic_intt_value[1]);

    //LOG_INFO("[%s]: dyn_intt_raw=%ld \r\n", __func__, dyn_intt_raw);

    epl_sensor.als.dyn_intt_raw =
		(dyn_intt_raw > 0xffff) ? 0xffff : dyn_intt_raw;
...
static int epl_sensor_get_als_value(struct epl_sensor_priv *obj, u16 als)
{
	...
	case CMC_BIT_DYN_INT:
		if (epl_sensor.als.lsrc_type != CMC_BIT_LSRC_NON) {
			...
			//lux ratio (A=CH1/CH0)
			luxratio =
				(long)((als * dynamic_intt_min_unit) / epl_sensor.als.data.channels[0]);
			...
			if ((epl_sensor.als.saturation >> 5) == 0) {
				...
				LOG_INFO("[%s]:ch0=%d, ch1=%d, c_gain=%d, obj->ratio=%d, obj->last_ratio=%d \r\n\n",
					...
					als, c_gain, obj->ratio, obj->last_ratio);
			} else {
...
int epl_sensor_read_als(struct i2c_client *client)
{
...
#endif
    {
    	...
    	epl_sensor.als.data.channels[1] =
			(gRawData.raw_bytes[3] << 8) | gRawData.raw_bytes[2];
...
{
	struct epl_sensor_priv *epld = epl_sensor_obj;
	int ps_state_code = (epl_sensor.ps.compare_low >> 3) ? FAR_CODE : NEAR_CODE;
	epl_ps_report_dist(epld, ps_state_code);
	dev_err(&epld->client->dev, "%s(): %s\n", __func__,
		(ps_state_code == FAR_CODE) ? "NEAR_TO_FAR" : "FAR_TO_NEAR");
	if ((FAR_CODE == ps_state_code) && !wake_lock_active(&epld->ps_lock)) {
...
int epl_sensor_read_ps_status(struct i2c_client *client)
{
    ...
    if (enable_ges == 1 && epl_sensor.ges.polling_mode == 1) {
        if ((new_ps_state == 0)
			&& (epl_sensor.ps.compare_low >> 3 == 1)
			&& (ps_ges_suspend_flag == false)
			&& (ges_saturation == 0)
			&& (ges_interrupt_flag == 0x04))
            epl_sensor_notify_event();
    } else if ((enable_ges == 1)
    	&& (epl_sensor.ges.polling_mode == 0)
    	&& (ps_ges_suspend_flag == false)
    	&& (ges_saturation == 0)
    	&& (ges_interrupt_flag == 0x04)) {
        if (new_ps_state == 0)
		...
		u16 low_thd, u16 high_thd)
{
	...
	err = i2c_smbus_write_word_data(client, 0x0C, low_thd);
	if (err < 0) {
	...
	err = i2c_smbus_write_word_data(client, 0x0E, high_thd);
	if (err < 0) {
...
static void write_global_variable(struct i2c_client *client)
{
...
#endif  /*HS_ENABLE*/
    {
        ...
        epl_sensor_I2C_Write(client, 0x22, 0x0);
        epl_sensor_I2C_Write(client, 0x23, 0x0);
        set_psensor_intr_threshold(epl_sensor.ges.low_threshold,
			epl_sensor.ges.high_threshold);
	} else
#endif
    {
    	/*ps setting*/
		...
		struct epl_sensor_priv *obj)
{
	...
	if (rc) {
		dev_err(&client->dev, "Unable to read ps_close_thd_set. ret = %d\n", rc);
		epl_sensor.ps.high_threshold = PS_HIGH_THRESHOLD;
	} else {
		epl_sensor.ps.high_threshold = (uint16_t)temp_val;
	}

	rc = of_property_read_u32(np, "epl,ps_away_thd_set", &temp_val);
	if (rc) {
		dev_err(&client->dev, "Unable to read ps_away_thd_set. ret = %d\n", rc);
		epl_sensor.ps.low_threshold = PS_LOW_THRESHOLD;
	} else {
		epl_sensor.ps.low_threshold = (uint16_t)temp_val;
	}

	rc = of_property_read_u32(np, "epl,ps_crosstalk_max", &temp_val);
	if (rc) {
		dev_err(&client->dev, "ps_crosstalk_max do not set and use default value%d\n",
				PS_MAX_XTALK);
	} else {
		PS_MAX_XTALK = (uint16_t)temp_val;
	}
	...
	epl_sensor.wait = EPL_WAIT_0_MS;	//20
	...
	epl_sensor.ps.cancelation = 0xFFFF;
	//ps factory
	epl_sensor.ps.factory.calibration_enable = false;
    ...
    set_als_ps_intr_type(client,
    	epl_sensor.ps.polling_mode, epl_sensor.als.polling_mode);
	...
	if (enable_ps == true) {
...
#endif
		{
...
#endif
		} else if ((dynk_change_flag == true)
			&& (epl_sensor.ps.data.data > dynk_change_thd_max)
			&& ((epl_sensor.ps.compare_low >> 3) ==0)) {
            ...
            LOG_INFO("[%s]: epl_sensor.ps.data.data=%d, L/H=%ld/%ld \r\n", __func__,
            	epl_sensor.ps.data.data, (long)dynk_thd_low, (long)dynk_thd_high);
#endif
        }
#if PS_DYN_K_STR
		if (polling_flag == true && eint_flag == true) {
...
#endif
		schedule_delayed_work(&obj->dynk_thd_polling_work,
...
void epl_sensor_update_mode(struct i2c_client *client)
{
...
#endif
    {
        ...
        epl_sensor_I2C_Write(epld->client, 0x02,
			epl_sensor.als.adc | epl_sensor.als.cycle);
        set_als_ps_intr_type(epld->client,
			epl_sensor.ps.polling_mode, epl_sensor.als.polling_mode);

        epl_sensor_I2C_Write(epld->client, 0x06,
			epl_sensor.interrupt_control | epl_sensor.ps.persist | epl_sensor.ps.interrupt_type);

        epl_sensor_I2C_Write(epld->client, 0x07,
			epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | epl_sensor.als.interrupt_type);
        ...
        if (enable_ges) {
            LOG_INFO("[%s] GES:low_thd = %d, high_thd = %d \n", __func__,
				epl_sensor.ges.low_threshold, epl_sensor.ges.high_threshold);
        ...
        if ((enable_als == 1 && epl_sensor.als.polling_mode == 1)
			...
			|| (enable_ges == 1 && epl_sensor.ges.polling_mode == 1))
#else
        if ((enable_als == 1 && epl_sensor.als.polling_mode == 1)
			|| (enable_ps==1 && epl_sensor.ps.polling_mode==1))
#endif
...
static irqreturn_t epl_sensor_eint_func(int irqNo, void *handle)
{
	...
	if (enable_ps)
		...
		dev_err(&epld->client->dev,
			"%s(): Irq triggered when ps disabled, do nothing!\n", __func__);
...
static void epl_sensor_eint_work(struct work_struct *work)
{
	...
	if (enable_ps && epl_sensor.ps.polling_mode == 0) {
		...
		if (epl_sensor.ps.interrupt_flag == EPL_INT_TRIGGER) {
...
#if PS_DYN_K
			epl_sensor_reset_dynk_thd(dynk_last_status,
            ...
            if (polling_flag == true) {
        		...
        		epl_sensor_I2C_Write(epld->client,
					0x1b, epl_sensor.ps.compare_reset | epl_sensor.ps.lock);
            } else {
                low_trigger_flag = true;
            }
		}
        ...
        if (enable_als == 0 && epl_sensor.als.polling_mode == 0) {
    		...
    		epl_sensor.als.lock = EPL_UN_LOCK;
    		epl_sensor_I2C_Write(epld->client,
				0x12, epl_sensor.als.compare_reset | epl_sensor.als.lock);
        }
#endif
	}
...
static DEVICE_ATTR(elan_reg, 0400, epl_sensor_show_reg, NULL);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (enable_als) {
        ...
        if (epl_sensor.als.lsrc_type != CMC_BIT_LSRC_NON) {
            len += snprintf(buf+len, PAGE_SIZE-len,
				"lsource_thd_low=%d, lsource_thd_high=%d \n",
        ...
        if (epl_sensor.als.report_type == CMC_BIT_DYN_INT) {
            len += snprintf(buf+len, PAGE_SIZE-len,
				"c_gain = %d\n", c_gain);
            len += snprintf(buf+len, PAGE_SIZE-len,
				"epl_sensor.als.dyn_intt_raw = %d, dynamic_intt_lux = %d\n",
...
static DEVICE_ATTR(elan_status, 0400, epl_sensor_show_status, NULL);

static void epl_als_update_enable(uint16_t mode)
{
	struct epl_sensor_priv *epld = epl_sensor_obj;
	LOG_FUN();
...
//    struct epl_sensor_priv *epld = epl_sensor_obj;
	...
	epl_als_update_enable(mode);
    return count;
}
static DEVICE_ATTR(als_enable, 0200, NULL, epl_sensor_store_als_enable);

static void epl_ps_update_enable(uint16_t mode)
{
	struct epl_sensor_priv *epld = epl_sensor_obj;
#if HS_ENABLE
	bool enable_hs = epld->enable_hflag == 1 && epld->hs_suspend == 0;
	...
	if (epld->enable_pflag != mode) {
		...
		epl_ps_update_intr_thds(epld->client, 0x00, 0xFFFF);
		epl_sensor.ps.cancelation = 0;

		epl_sensor_update_mode(epld->client);
		if (mode) {
			epl_run_ps_calibration(epld);
#if PS_DYN_K
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    sscanf(buf, "%hu", &mode);
	epl_ps_update_enable(mode);

    return count;
}
static DEVICE_ATTR(ps_enable, 0200, NULL, epl_sensor_store_ps_enable);
...
static DEVICE_ATTR(cal_raw, 0400, epl_sensor_show_cal_raw, NULL);
...
static DEVICE_ATTR(wait_time, 0200, NULL, epl_sensor_store_wait_time);
...
static DEVICE_ATTR(mode, 0200, NULL, epl_sensor_store_mode);
...
static DEVICE_ATTR(ir_mode, 0200, NULL, epl_sensor_store_ir_mode);
...
static DEVICE_ATTR(ir_on, 0200, NULL, epl_sensor_store_ir_contrl);
...
static DEVICE_ATTR(ir_drive, 0200, NULL, epl_sensor_store_ir_drive);
...
static DEVICE_ATTR(interrupt_type, 0200, NULL, epl_sensor_store_interrupt_type);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case EPL_MODE_PS: //ps
		...
		LOG_INFO("[%s]:0x04 = 0x%x (0x%x)\n", __FUNCTION__,
			epl_sensor.ps.adc | epl_sensor.ps.cycle, gRawData.raw_bytes[0]);
		break;
	case EPL_MODE_ALS: //als
		...
		LOG_INFO("[%s]:0x02 = 0x%x (0x%x)\n", __FUNCTION__,
			epl_sensor.als.adc | epl_sensor.als.cycle, gRawData.raw_bytes[0]);
		break;
	}
...
static DEVICE_ATTR(adc, 0200, NULL, epl_sensor_store_adc);
...
static ssize_t epl_sensor_store_ps_w_calfile(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    ps_cal_len = sprintf(ps_calibration, "%d,%d,%d",
		ps_cancelation, ps_hthr, ps_lthr);
...
static DEVICE_ATTR(unlock, 0200, NULL, epl_sensor_store_unlock);
...
static DEVICE_ATTR(als_ch, 0200, NULL, epl_sensor_store_als_ch_sel);
...
static DEVICE_ATTR(ps_cancel, 0200, NULL, epl_sensor_store_ps_cancelation);
...
static void epl_run_ps_calibration(struct epl_sensor_priv *epl_data)
{
    ...
    ch1 = (u16)(ch1_all / count);
	dev_err(&epld->client->dev, "%s(): get a_CH1 data: %d\n", __func__, ch1);
    if ((ch1 < PS_MAX_XTALK) && (ch1 >= 0)) {
		epl_sensor.ps.cancelation = ch1;
    } else {
        dev_err(&epld->client->dev, 
			"[%s] Warning! : ch1(%d) > max_xtalk(%d) \r\n",
			__func__, ch1, PS_MAX_XTALK);
		if (epl_sensor.ps.cancelation != 0xFFFF) {
			ch1 = epl_sensor.ps.cancelation;
		}
	}
	dev_err(&epld->client->dev, "%s(): get ps_canc: %d\n",
		__func__, epl_sensor.ps.cancelation);
	if (epl_sensor.ps.cancelation != 0xFFFF) {
		ps_hthr = ch1 + epl_sensor.ps.high_threshold;
		ps_lthr = ch1 + epl_sensor.ps.low_threshold;
	} else {
		ps_hthr = 0xFFFF;
		ps_lthr = 0x0;
	}
    ...
    //ps_cal_len =
	//	sprintf(ps_calibration, "%d,%d,%d", ps_cancelation, ps_hthr, ps_lthr);
	...
	set_psensor_intr_threshold(ps_lthr, ps_hthr);
...
static DEVICE_ATTR(run_ps_cali, 0400, epl_sensor_show_ps_run_cali, NULL);
...
static DEVICE_ATTR(als_dyn_lsrc_type, 0200, NULL,epl_sensor_store_lsrc_type);
...
static DEVICE_ATTR(als_dyn_lsrc_thd, 0200, NULL, epl_sensor_store_lsrc_thd);
#endif

#if PS_DYN_K
static ssize_t epl_sensor_store_dyn_offset(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    int dyn_h,dyn_l;
    LOG_FUN();
...
static ssize_t epl_sensor_store_dyn_thd_offset(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    int thd_offset;
    LOG_FUN();
...
static DEVICE_ATTR(dyn_thd_offset, 0200, NULL, epl_sensor_store_dyn_thd_offset);

static ssize_t epl_sensor_store_dyn_change_thd_max(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    int thd_max;
    LOG_FUN();
...
static DEVICE_ATTR(dyn_change_max, 0200, NULL, epl_sensor_store_dyn_change_thd_max);

static ssize_t epl_sensor_store_dyn_max_ir_data(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    int max_ir_data;
    LOG_FUN();
...
#if PS_DYN_K_STR
static ssize_t epl_sensor_store_dyn_enhance_max_ch0(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    //struct epl_sensor_priv *epld = epl_sensor_obj;
    int max_ch0;
...
static DEVICE_ATTR(dyn_enh_max_ch0, 0200, NULL, epl_sensor_store_dyn_enhance_max_ch0);

static ssize_t epl_sensor_store_dyn_enhance_gain_intt(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    //struct epl_sensor_priv *epld = epl_sensor_obj;
    int gain, intt;
...
static DEVICE_ATTR(dyn_enh_gain_intt, 0200, NULL, epl_sensor_store_dyn_enhance_gain_intt);
#endif
#endif
...
static DEVICE_ATTR(elan_renvo, 0400, epl_sensor_show_renvo, NULL);

static ssize_t epl_sensor_store_hs_enable(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    uint16_t mode=0;
    struct epl_sensor_priv *obj = epl_sensor_obj;
    ...
    if (enable_ps == 0) {
        if (mode > 0) {
            ...
            epl_sensor.hs.dynamic_intt = (mode == 2) ? false : true;
    	} else {
            obj->enable_hflag = 0;
            if (hs_enable_flag == true) {
                obj->enable_lflag = 1;
...
static ssize_t epl_sensor_store_hs_int_time(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    struct epl_sensor_priv *obj = epl_sensor_obj;
	int value;
...
static ssize_t epl_sensor_show_hs_raws(struct device *dev,
		struct device_attribute *attr, char *buf)
{
    u16 *tmp = (u16*)buf;
...
static DEVICE_ATTR(hs_raws, 0400, epl_sensor_show_hs_raws, NULL);
#endif

#if PS_GES
static ssize_t epl_sensor_store_ges_enable(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    struct epl_sensor_priv *obj = epl_sensor_obj;
    ...
    if (enable_ps == 0) {
		obj->enable_gflag = (ges_enable == 1) ? 1 : 0;
    }
...
static ssize_t epl_sensor_store_ges_polling_mode(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    struct epl_sensor_priv *obj = epl_sensor_obj;
    struct i2c_client *client = obj->client;
...
static ssize_t epl_sensor_store_ges_thd(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    int ges_l=0, ges_h=0;
    struct epl_sensor_priv *obj = epl_sensor_obj;
...
static int epl_sensor_setup_lsensor(struct epl_sensor_priv *epld)
{
	...
	sensors_light_cdev.sensors_enable = epl_als_set_enable;
	sensors_light_cdev.sensors_poll_delay = epl_als_set_delay;
	err = sensors_classdev_register(&epld->als_input_dev->dev, &sensors_light_cdev);
...
static DEVICE_ATTR(ps_data, 0440, epl_sensor_show_pdata, NULL);

static ssize_t epl_sensor_ps_thd_away_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", epl_sensor.ps.low_threshold);
}

static ssize_t epl_sensor_ps_thd_away_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct epl_sensor_priv *epld = epl_sensor_obj;
    int threshold = 0;
	...
	epl_sensor.ps.low_threshold = threshold;
	set_psensor_intr_threshold(epl_sensor.ps.low_threshold,
		epl_sensor.ps.high_threshold);
...
static DEVICE_ATTR(ps_thd_away, 0640,
	epl_sensor_ps_thd_away_show, epl_sensor_ps_thd_away_store);

static ssize_t epl_sensor_ps_thd_close_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", epl_sensor.ps.high_threshold);
}

static ssize_t epl_sensor_ps_thd_close_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct epl_sensor_priv *epld = epl_sensor_obj;
    int threshold = 0;
	...
	sscanf(buf, "%d", &threshold);
	epl_sensor.ps.high_threshold = threshold;
	set_psensor_intr_threshold(epl_sensor.ps.low_threshold,
		epl_sensor.ps.high_threshold);
...
static DEVICE_ATTR(ps_thd_close, 0640,
	epl_sensor_ps_thd_close_show, epl_sensor_ps_thd_close_store);

static ssize_t epl_sensor_ps_canc_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", epl_sensor.ps.cancelation);
}
static DEVICE_ATTR(ps_canc, 0440, epl_sensor_ps_canc_show, NULL);

static ssize_t epl_sensor_ps_canc_max_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", PS_MAX_XTALK);
}

static ssize_t epl_sensor_ps_canc_max_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct epl_sensor_priv *epld = epl_sensor_obj;
    int threshold = 0;
	...
	sscanf(buf, "%d", &threshold);
	PS_MAX_XTALK = threshold;

	return count;
}
static DEVICE_ATTR(ps_crosstalk_maxthd, 0640,
	epl_sensor_ps_canc_max_show, epl_sensor_ps_canc_max_store);
...
static int epl_sensor_setup_psensor(struct epl_sensor_priv *epld)
{
	...
	sensors_proximity_cdev.sensors_enable = epl_ps_set_enable;
	sensors_proximity_cdev.sensors_poll_delay = epl_ps_set_delay;
	err = sensors_classdev_register(&epld->ps_input_dev->dev,
		&sensors_proximity_cdev);

    err = sysfs_create_group(&sensors_proximity_cdev.dev->kobj, &proximity_attribute_group);
	if (err) {
		...
		const struct i2c_device_id *id)
{
    ...
    err = i2c_smbus_read_byte_data(client, 0x21);
	err &= 0xFF;
	if ((err != 0x81) && (err != 0x91)) {
