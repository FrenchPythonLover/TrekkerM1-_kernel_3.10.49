...
static int is_runtime_resume_sd = 0;

void set_runtime_resume_sd(int status)
{
	is_runtime_resume_sd = !!status;
	//printk("[host_runst] status = %d\n", is_runtime_resume_sd);
}
EXPORT_SYMBOL(set_runtime_resume_sd);

int is_runtime_resume_sd_func(void)
{
	if(is_runtime_resume_sd == 1)
		return 1;
	else
		return 0;	
}
EXPORT_SYMBOL(is_runtime_resume_sd_func);
...
#ifdef CONFIG_SDCARD_RESUME_OPT //wangyongqing
	if(host->index == 1) //sdcard
		set_runtime_resume_sd(1);
#endif	
...
static int mmc_host_resume(struct device *dev)
{
	...
	host->dev_status = DEV_RESUMED;
       ...
       printk(KERN_INFO "                      ===== mmc_host_resume cost %lld usecs\n", duration);
#endif
