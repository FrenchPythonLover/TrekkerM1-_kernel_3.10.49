...
#ifdef CONFIG_SDCARD_RESUME_OPT
int first_resume_sd = 1; //wangyongqing
EXPORT_SYMBOL(first_resume_sd);
static int is_sd_card(struct mmc_host *host);
extern int is_pwrkey_resume_sys(void);
extern int is_runtime_resume_sd_func(void);
extern void set_runtime_resume_sd(int status);
#endif
...
#ifdef CONFIG_SDCARD_RESUME_OPT  //wangyongqing	
	if(is_sd_card(host))
	{
		if(first_resume_sd == 0) //not the first power on stage
...
#ifdef CONFIG_SDCARD_RESUME_OPT_DEBUG 
	printk(KERN_ERR "+++ mmc_rescan enter\n");
#endif
...
#ifdef CONFIG_SDCARD_RESUME_OPT_DEBUG
	printk(KERN_ERR "+++ mmc_rescan leave\n");
#endif
}
...
struct mmc_host *sd_mmc_host_hmct = NULL;
static int sd_resume_complete = 0;

static void sd_resume_work(struct work_struct *work)
{
	struct mmc_host *host = sd_mmc_host_hmct;
	int err = 0;
	unsigned long flags;

#ifdef CONFIG_SDCARD_RESUME_OPT_DEBUG
	printk(KERN_ERR "\n@@@ enter sd_resume_work\n");
#endif
	...
	spin_lock_irqsave(&host->lock, flags);
	sd_resume_complete = 0;
	host->rescan_disable = 1; //if disable = 1, will forbid the mmc_rescan function.
	spin_unlock_irqrestore(&host->lock, flags);
	...
	spin_lock_irqsave(&host->lock, flags);
	host->rescan_disable = 0; //if disable = 1, will forbid the mmc_rescan function.
	sd_resume_complete = 1;
	spin_unlock_irqrestore(&host->lock, flags);
	//mmc_detect_change(host, 0);

#ifdef CONFIG_SDCARD_RESUME_OPT_DEBUG
	printk(KERN_ERR "\n@@@ leave sd_resume_work\n");
#endif

	return;
}

int sd_resume_init(void)
{
	int err = 0;
	INIT_WORK(&resume_work, sd_resume_work);
	return err;
}
...
int mmc_resume_host(struct mmc_host *host)
{
...
#ifdef CONFIG_SDCARD_RESUME_OPT //wangyongqing
		if( is_sd_card(host)) //wangyongqing added 
		{
			if((!first_resume_sd) && (is_pwrkey_resume_sys() == 1) 
				&& (0 == is_runtime_resume_sd_func())) //It's not the first time to power on the card
			{
				//printk("[mmc_resume_host] pwrkey resume sdcard\n");
#ifdef CONFIG_SDCARD_RESUME_OPT_DEBUG
				printk("			<<<<<<< will enter sd resume work\n");
#endif
				sd_mmc_host_hmct = host;
				spin_lock_irqsave(&host->lock, flags);
				host->rescan_disable = 1;
				spin_unlock_irqrestore(&host->lock, flags);
			...
			else if(is_runtime_resume_sd_func())
			{
				set_runtime_resume_sd(0);
			}
	...
	if(is_sd_card(host)) //wangyongqing added
	{
		if(first_resume_sd == 1)
		{
			first_resume_sd = 0;
#ifdef CONFIG_SDCARD_RESUME_OPT_DEBUG			
			printk("\n         +++++ first power on leave mmc_resume_host\n");
#endif
		}
	}
					...
					unsigned long mode, void *unused)
{
	...
	case PM_POST_RESTORE:
		...
		host->rescan_disable = 0;
#ifdef CONFIG_SDCARD_RESUME_OPT  //wangyongqing
		if((is_pwrkey_resume_sys() == 1) && is_sd_card(host))
		{
			if(sd_resume_complete == 0)//the resume work don't complete
			{
				spin_unlock_irqrestore(&host->lock, flags);
				//printk("[mmc_pm_notify] delay 500ms to detect\n");
				mmc_detect_change(host, msecs_to_jiffies(200));
				break;
			}
			else
			{
				spin_unlock_irqrestore(&host->lock, flags);
				mmc_detect_change(host, 0);
				//printk("[mmc_pm_notify] delay 0ms to detect\n");
				break;
			}
		}
		else
