...
#include "mmc_ops.h"
#include "sd_ops.h"
#define DEBUG_MMC_SLEEP 1
#include <linux/mmc/samsung_ffu_p13_16g.h>
#include <linux/mmc/samsung_ffu_p06_64g.h>
#include <linux/mmc/ffu.h>
#ifdef CONFIG_MMC_FFU
#define  KMQE10013M_B318    "QE13MB"
#define  KMFNX0012M_B214    "FNX2MB"
#define  KMQE10013M_B512    "RE1BMB"
#define  P13                0xd
#define  P06                0x6
int ffu_first_down=1;
...
void mmc_add_emmcinfo_to_productinfo(struct mmc_card *card, u8 *ext_csd)
{
	...
	mdt_year = (int)(card->cid.year) - 1997;
	if (m_ext_csd_rev > 4) mdt_year += 2013;
	else
		mdt_year += 1997;
	...
	mdt_year, mdt_month);
	snprintf(emmc_more, sizeof(emmc_more), "PSN %x", card->cid.serial);
...
#ifdef CONFIG_MMC_FFU	
	printk(KERN_ERR" MID 0x%x, PNM %s, ", card->cid.manfid,card->cid.prod_name);
	if(!mmc_card_is_removable(host) && (card->cid.manfid == CID_MANFID_SAMSUNG) 
			&& !( strncmp(card->cid.prod_name, KMQE10013M_B318,sizeof(KMQE10013M_B318))
			&& strncmp(card->cid.prod_name, KMFNX0012M_B214,sizeof(KMFNX0012M_B214)) 
			&& strncmp(card->cid.prod_name, KMQE10013M_B512,sizeof(KMQE10013M_B512)))){
		struct mmc_command cmd;
		int err;
		void *emmc_fw_data;
		char emmc_fw_version;
		long unsigned int emmc_fw_data_size;
		...
		if(!(strncmp(card->cid.prod_name, KMQE10013M_B318,sizeof(KMQE10013M_B318)) 
			&& strncmp(card->cid.prod_name, KMQE10013M_B512,sizeof(KMQE10013M_B512)))){
			emmc_fw_data = eMMC_FW_P13_16G;
			emmc_fw_data_size = sizeof(eMMC_FW_P13_16G);
			emmc_fw_version   = P13;
		}
		else if(!strncmp(card->cid.prod_name, KMFNX0012M_B214,sizeof(KMFNX0012M_B214))){
			emmc_fw_data = eMMC_FW_P06_64G;
			emmc_fw_data_size = sizeof(eMMC_FW_P06_64G);
			emmc_fw_version   = P06;
		}
		printk("ffu size is %lu byte FWV %x mmc data size %lu\n",
			emmc_fw_data_size,ext_csd[EXT_CSD_FIRMWARE_VERSION],emmc_fw_data_size);
		if(ext_csd[EXT_CSD_FIRMWARE_VERSION] < emmc_fw_version && ffu_first_down){
				err = mmc_ffu_download(card, &cmd , emmc_fw_data ,
					emmc_fw_data_size);
				pr_err("ffu download err %d\n",err);
				if(err){
					pr_err("ffu download err %d\n",err);
					goto SKIP_FFU;
				}
...
static int mmc_resume(struct mmc_host *host)
{
...
#ifdef CONFIG_SDCARD_RESUME_OPT_DEBUG //wangyongqing
	...
	if (mmc_can_scale_clk(host))
       ...
       printk(KERN_INFO "                      ===== mmc_resume cost %lld usecs\n", duration);
#endif
#ifdef DEBUG_MMC_SLEEP
    printk("%s:%d  end\n",__func__,__LINE__);
#endif
