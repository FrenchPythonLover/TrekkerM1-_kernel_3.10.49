...
extern int qpnp_pon_poweron_alarm(bool poweroff_alarm);//hmct
extern long get_power_on_alarm_time(void);//hmct
...
qpnp_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	unsigned long alarm_time;//hmct
	alarm_time = get_power_on_alarm_time();//hmct
	rtc_tm_to_time(&alarm->time, &secs);

	// add hmct:setting register for alarm power on
	if(alarm_time == secs) {
		qpnp_pon_poweron_alarm(true);
		dev_err(dev,"qpnp rtc set power on alarm %lu\n",secs);
	}
	else {
		dev_err(dev,"qpnp rtc not power on alarm %lu : %lu\n",secs,alarm_time);
		qpnp_pon_poweron_alarm(false);
	}
	// add end
	/*
	...
	pr_debug("qpnp rtc %lu,expire %lu\n",secs_rtc,alarm_time-secs_rtc);
