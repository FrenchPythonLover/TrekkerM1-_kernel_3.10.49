...
static int sys_resume_reason = 0;  //if resume by powerkey is 1, then 0

void set_sys_resume_reason_pwrkey(int status)
{
	sys_resume_reason = !!status;
	//printk("[qpnp-int] status = %d\n", sys_resume_reason);
}
EXPORT_SYMBOL(set_sys_resume_reason_pwrkey);

int is_pwrkey_resume_sys(void)
{
	if(sys_resume_reason == 1)
		return 1;
	else
		return 0;
}
EXPORT_SYMBOL(is_pwrkey_resume_sys);
...
#ifdef CONFIG_SDCARD_RESUME_OPT  //wangyongqing
		if((spec->slave == 0) && (spec->per == 8) && (spec->irq == 0)) //equal
			set_sys_resume_reason_pwrkey(1);
		else
			set_sys_resume_reason_pwrkey(0);	
#endif		
	} else {
