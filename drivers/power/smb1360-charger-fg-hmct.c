...
#define OTG_UVLO_VOLTAGE		0x12
#define OTG_UVLO_THRESH_MASK	SMB1360_MASK(4, 2)
#define OTG_UVLO_THRESH_SHIFT	2

#define OTP_PROFILE_A_REG	0x00
#define OTP_CSIR_2243		0x0D
#define OTP_CSIR_2289		0xE8

#define OTP_CSIR_NUM_FIRST_REG 0xD2
#define OTP_CSIR_NUM_LAST_REG 0xD3
#define OTP_CSIR_2337_FIRST		0x23
#define OTP_CSIR_2337_LAST		0x37

#define CSIR_2337		0x2337	/*smb1360-29*/
#define CSIR_2334		0x2334	/*smb1360-28*/
#define CSIR_2289		0x2289	/*smb1360-15*/
#define CSIR_2243		0x2243	/*smb1360-13*/
...
#define AICL_CURRENT_MASK	SMB1360_MASK(3, 0)
...
struct smb1360_chip {
	...
	struct delayed_work		capacity_work;
	struct delayed_work		otg_reconnect_work;
	...
	int				cold_hysteresis;
	int				hot_hysteresis;
    bool			soc_calibration_flag;
    bool            switch_profile_flag;
	int				input_current_value;
};
...
static int aicl_current_status[] = {
...
static int otg_uvlo_mv[] = {2750, 2860, 3190, 3300, 3410, 3740};
static int otg_oc;
static int is_otg_enabled;
static bool vbat_is_dead;
extern int boot_charger_status;
static int stop_charge_expired = 1;

module_param(stop_charge_expired, int, 0644);
static int timeout_set_flag = 1;
	...
	enum wakeup_src wk_src)
{
	...
	if (!__test_and_set_bit(wk_src, &source->enabled_bitmap)) {
		...
		pr_info("enabled source %s, wakeup_src %d\n",
...
static void smb1360_dump_regs(struct smb1360_chip *chip)
{
	int rc;
	u8 reg;
	u8 addr;

	pr_info("-----CONFIGS REGS-----\n");
...
static int smb1360_force_fg_reset(struct smb1360_chip *chip)
{
    ...
    pr_info("smb1360_force_fg_reset start\n");
    ...
    pr_info("smb1360_force_fg_reset end\n");

    chip->soc_calibration_flag = true;
...
static int smb1360_get_prop_charge_status(struct smb1360_chip *chip)
{
	int rc;
	u8 reg = 0, chg_type;
...
static int smb1360_get_prop_batt_capacity(struct smb1360_chip *chip)
{
	...
	if (chip->batt_present) {
		...
		if (chip->usb_present) {
			/* Always report 100 if real soc > 96
			* during recharge process,or show the truth
			*/
			UI_SOC = bound(soc, 0, 100);
			if (chip->batt_full) {
				/* 99-3(fg_auto_recharge_soc-3) */
				if (UI_SOC  > 96)
					UI_SOC = 100;
				else
			...
			if (chip->soc_calibration_flag) {
				chip->soc_calibration_flag = false;
				UI_SOC = bound(soc, 0, 100);
				pr_info("fg_soc=%d, batt_full = %d\n",
					soc, chip->batt_full);
			}

			if (soc < UI_SOC)
				UI_SOC--;
			else if ((UI_SOC == 99) && (soc == 100))
				pr_debug("prevent UI_SOC from  changing to 100 when usb removed\n");
			else
				UI_SOC = soc;

			/* chip->batt_full = (UI_SOC == 100) ? true : false; */
			chip->batt_full = false;
			chip->chg_term = false;
		}
	} else {
		UI_SOC = DEFAULT_CAPACITY;
	}
	chip->soc_now = UI_SOC;
	return UI_SOC;
}
...
static int smb1360_get_prop_batt_temp(struct smb1360_chip *chip)
{
	...
	batt_temp = (reg[1] << 8) | reg[0];
	/* temperature in kelvin */
	batt_temp = div_u64(batt_temp * 625, 10000UL);
	/* temperature in decideg */
	batt_temp = (batt_temp - 273) * 10;

	pr_debug("reg[0]=0x%02x reg[1]=0x%02x temperature=%d\n",
					reg[0], reg[1], batt_temp);
	/*Filter:Be more careful to report >= 680*/
	if (batt_temp >= MSM_ANDROID_SDN_LIMITED) {
		get_monotonic_boottime(&ts);
		interval_ot = ts.tv_sec - last_ot_secs;

        pr_info("interval_ot:%ld batt_temp:%d count:%d\n",
            interval_ot, batt_temp, battery_hot_count);

		if (interval_ot >=  1) {
			/*only count for > 1s(unplug report changes too much)*/
			battery_hot_count++;
            smb1360_stay_awake(&chip->smb1360_ws, WAKEUP_SRC_OVERHEAT);
			last_ot_secs = ts.tv_sec;
		}

        if (battery_hot_count <= MSM_BAT_TEMP_SDN_COUNT)
            batt_temp = MSM_ANDROID_SDN_LIMITED - 20;
	} else if (battery_hot_count != 0) {
		battery_hot_count = 0;
        smb1360_relax(&chip->smb1360_ws, WAKEUP_SRC_OVERHEAT);
	}

	chip->temp_now = batt_temp;
...
static void smb1360_parallel_work(struct work_struct *work)
{
	...
	if (chip->input_current_value >= 3)
		chip->input_current_value -= 3;

	if ((reg & AICL_CURRENT_MASK) >=
		aicl_current_status[chip->input_current_value]) {
...
static int smb1360_set_appropriate_usb_current(struct smb1360_chip *chip)
{
	...
	if (current_ma <= CURRENT_100_MA) {
		...
		if (i < 0) {
			...
			i = 0;
		} else if (i < (ARRAY_SIZE(fastchg_current) - 1))
			i++;
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TIMER_ENABLED:
		stop_charge_expired = val->intval;
		break;
	default:
		return -EINVAL;
	}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HEALTH:
		if(vbat_is_dead)
			val->intval = POWER_SUPPLY_HEALTH_DEAD;//hmct
		else
	...
	case POWER_SUPPLY_PROP_CHARGE_STATUS:
		val->intval = smb1360_get_prop_charge_status(chip);
		break;
	case POWER_SUPPLY_PROP_CHARGE_TIMER_ENABLED:
		val->intval = stop_charge_expired;
		break;
	default:
		return -EINVAL;
	}
...
static void smb1360_jeita_work_fn(struct work_struct *work)
{
    ...
    pr_info("hot %d, cold %d, warm %d, cool %d\n",
        chip->batt_hot, chip->batt_cold, chip->batt_cool, chip->batt_cool);
    pr_info("soft_cold_rt_sts %d, soft_hot_rt_sts %d, threshold_now %d %d\n",
        !!chip->soft_cold_rt_stat, !!chip->soft_hot_rt_stat,
...
{
	chip->chg_term = !!rt_stat;

	if ((UI_SOC == 100) && chip->usb_present)
		chip->batt_full = true;
	else
		chip->batt_full = !!rt_stat;

	pr_info("rt_stat = 0x%02x,batt_full %d,chg_term %d\n",
		rt_stat, chip->batt_full, chip->chg_term);
...
{
	if(is_otg_enabled == 1) {
		char *otg_oc_event[2]	  = { "OTG_DEVICE=OC", NULL };

		pr_info("otg_oc_handler send OTG_DEVICE=OC uevent\n");
		otg_oc = 1;
		kobject_uevent_env(&chip->dev->kobj, KOBJ_CHANGE, otg_oc_event);
		schedule_delayed_work(&chip->otg_reconnect_work, 3*HZ);
	} else {
		pr_info("otg is disabled, not handle this interrupt\n");
	}
	return 0;
}
...
static int smb1360_otg_regulator_enable(struct regulator_dev *rdev)
{
	...
	char *otg_on[2]	  = { "OTG_DEVICE=ON", NULL };

	rc = __smb1360_charging_disable(chip, true);
	if (rc)
		pr_err("Couldn't disable charging for rc=%d\n",rc);
	...
	mutex_unlock(&chip->otp_gain_lock);
	pr_info("smb1360_otg_regulator_enable send OTG_DEVICE=ON uevent\n");
    kobject_uevent_env(&chip->dev->kobj, KOBJ_CHANGE, otg_on);
	is_otg_enabled =1;
	return rc;
}
...
static int smb1360_otg_regulator_disable(struct regulator_dev *rdev)
{
	int rc = 0;
	struct smb1360_chip *chip = rdev_get_drvdata(rdev);
	char *otg_off[2]	  = { "OTG_DEVICE=OFF", NULL };

	rc = __smb1360_charging_disable(chip, false);
	if (rc)
		pr_err("Couldn't disable charging for rc=%d\n",rc);

    cancel_delayed_work_sync(&chip->otg_reconnect_work);

	rc = smb1360_otg_disable(chip);
	if (rc)
		pr_err("Couldn't disable OTG regulator rc=%d\n", rc);
	else {
		pr_info("smb1360_otg_regulator_enable send OTG_DEVICE=OFF uevent\n");
		kobject_uevent_env(&chip->dev->kobj, KOBJ_CHANGE, otg_off);
		is_otg_enabled =0;
		otg_oc = 0;
	}
...
#define CSIR2243_SWITCH_CAPACITY	3000
#define CSIR2289_SWITCH_CAPACITY	2500
#define CSIR2243_MAX_CAPACITY		4500
#define CSIR2289_MAX_CAPACITY		3000
#define CSIR2337_SWITCH_CAPACITY	2600
#define CSIR2337_MAX_CAPACITY		4800
#define CSIR2334_SWITCH_CAPACITY	3300
/*#define CSIR2334_MAX_CAPACITY		reserved*/

static bool smb1360_should_swith_to_profile_b(struct smb1360_chip *chip)
{
	if (chip->smb1360_ic_type == CSIR_2243) {
		if (chip->batt_capacity_mah > CSIR2243_SWITCH_CAPACITY) {
			if (chip->batt_capacity_mah > CSIR2243_MAX_CAPACITY)
				pr_info("CSIR2243 can't cover %d mAh\n",
					CSIR2243_MAX_CAPACITY);
			return true;
		}
	} else if (chip->smb1360_ic_type == CSIR_2337) {
		if (chip->batt_capacity_mah > CSIR2337_SWITCH_CAPACITY) {
			if (chip->batt_capacity_mah > CSIR2337_MAX_CAPACITY)
				pr_info("CSIR2337 can't cover %d mAh\n",
					CSIR2337_MAX_CAPACITY);
			return true;
		}
	} else if (chip->smb1360_ic_type == CSIR_2289) {
		if (chip->batt_capacity_mah > CSIR2289_SWITCH_CAPACITY) {
			if (chip->batt_capacity_mah > CSIR2289_MAX_CAPACITY)
				pr_info("CSIR2289 can't cover %d mAh\n",
					CSIR2289_MAX_CAPACITY);
			return true;
		}
	} else if (chip->smb1360_ic_type == CSIR_2334) {
		/*do nothing, becase 2334 only has profile a in msm's table.*/
	}
	return false;
}

static int smb1360_csir_predicate(struct smb1360_chip *chip)
{
	int rc = 0;
	u8 reg = 0;
	u8 value = 0;
	...
	rc = smb1360_fg_read(chip, OTP_CSIR_NUM_FIRST_REG, &reg);
	if (rc) {
		pr_err("Couldn't read FG reg rc = %d\n", rc);
		return rc;
	}

	rc = smb1360_fg_read(chip, OTP_CSIR_NUM_LAST_REG, &value);
	if (rc) {
		pr_err("Couldn't read FG reg rc = %d\n", rc);
		return rc;
	}
	...
	pr_info("smb1360 csir=0x%x\n", csir);

	if (CSIR_2337 == csir) {
		chip->smb1360_ic_type = CSIR_2337;
		goto restore_fg;
	} else if (CSIR_2334 == csir) {
		chip->smb1360_ic_type = CSIR_2334;
		goto restore_fg;
	}

	chip->fg_access_type = FG_ACCESS_PROFILE_A;
	...
	rc = smb1360_fg_read(chip, OTP_PROFILE_A_REG, &reg);
	if (rc) {
		pr_err("Couldn't read FG Profile A reg rc = %d\n", rc);
		return rc;
	}

	pr_info("Profile A Reg: 0x%x: 0x%x\n", OTP_PROFILE_A_REG, reg);

	if (reg == OTP_CSIR_2243)
		chip->smb1360_ic_type = CSIR_2243;
	else if (reg == OTP_CSIR_2289)
		chip->smb1360_ic_type = CSIR_2289;
...
static int smb1360_switch_to_profile_b(struct smb1360_chip *chip)
{
	int rc;
	u8 reg = 0, new_profile = 0, bid_mask;
	...
	if (reg == new_profile) {
		pr_info("Profile B already loaded\n");
		return 0;
	}

    chip->switch_profile_flag = true;

	/* Only switch to profile b
	* when smb is firstly powered(battery install) set the BID mask
	*/
	rc = smb1360_masked_write(chip, CFG_FG_BATT_CTRL_REG,
    ...
    while (timeout) {
        ...
        reg = !!(reg & BATTERY_PROFILE_BIT);
        if (reg == new_profile) {
            pr_info("new_profile = BATTERY_PROFILE_B loaded\n");
            break;
        }
        timeout--;
    }
...
static void smb1360_switch_profile(struct smb1360_chip *chip)
{
	int	rc;

	if (smb1360_should_swith_to_profile_b(chip)) {
		pr_info("switch to profile B\n");
		rc = smb1360_switch_to_profile_b(chip);
		if (rc)
			pr_err("Unable to modify battery profile\n");
	} else {
		pr_info("not change, keep profile A\n");
	}
}
...
static void smb_otg_reconnect_work(struct work_struct *work)
{
    int rc =0;
	struct smb1360_chip *chip = container_of((struct delayed_work *)work,
					struct smb1360_chip,otg_reconnect_work);
    char *otg_re[2]	  = { "OTG_DEVICE=RE", NULL };

	rc = smb1360_masked_write(chip, CMD_CHG_REG, CMD_OTG_EN_BIT, 0);
    if (rc)
	 pr_info("cannot clear OTG Enable bit after OC \n");

	rc = smb1360_masked_write(chip, CMD_CHG_REG, CMD_OTG_EN_BIT,
						CMD_OTG_EN_BIT);
	if (rc)
		pr_info("cannot set OTG Enable bit after OC \n");
	else {
		kobject_uevent_env(&chip->dev->kobj, KOBJ_CHANGE, otg_re);
		otg_oc = 0;
		pr_info("smb_otg_reconnect_work send OTG_DEVICE=RE uevent\n");
	}

}

static int smb1360_batt_voltage_exceed_delta(struct smb1360_chip *chip, int vbat_for_first)
{
	int voltage, curr_val;
	int bat_resist = 0, bat_ocv = 0;
	int delta_vol_threshold = 200;
    int rc = 0;

	if (chip->batt_present) {
        voltage = smb1360_get_prop_voltage_now(chip);
        curr_val = smb1360_get_prop_current_now(chip);
		bat_resist = smb1360_get_prop_batt_resistance(chip);

		bat_ocv = voltage + (curr_val/1000) * (bat_resist/1000);
		bat_ocv = bat_ocv/1000;

		pr_info("voltage = %d, curr_val = %d, bat_resist = %d, bat_ocv=%d\n",
			voltage/1000, curr_val/1000, bat_resist/1000, bat_ocv);

		if (abs(bat_ocv - vbat_for_first) > delta_vol_threshold)
			rc = 1;
    }
    return rc;
}

static int smb1360_fg_reset_condition(struct smb1360_chip *chip)
{
	int rc = 0, temp, timeout = 10;
	u8 reg = 0, reg2[2];
    int v_predicted, v_now;
    int exceed_delta_flag;
    ...
    exceed_delta_flag = smb1360_batt_voltage_exceed_delta(chip, v_predicted);
    if (((temp >= chip->fg_reset_threshold_mv) && chip->fg_reset_at_pon)
        || exceed_delta_flag || chip->switch_profile_flag){
...
disable_fg_reset:
    ...
    if (chip->switch_profile_flag) {
        ...
        chip->switch_profile_flag = false;
        if (!timeout) {
            pr_err("New profile could not be loaded\n");
            return -EBUSY;
        }
...
static int smb1360_fg_config(struct smb1360_chip *chip)
{
    ...
    smb1360_fg_reset_condition(chip);
...
static void smb1360_check_feature_support(struct smb1360_chip *chip)
{
	...
	chip->workaround_flags |= WRKRND_USB100_FAIL; //hs
	/*
	 * FG Configuration
	 *
...
static int smb1360_hw_init(struct smb1360_chip *chip)
{
	...
	if (chip->safety_time != -EINVAL) {
		if (chip->safety_time == 0) {
			...
			rc = smb1360_masked_write(chip, CFG_SFY_TIMER_CTRL_REG,
				SAFETY_TIME_DISABLE_BIT|SAFETY_TIME_EN_BIT|SAFETY_TIME_MINUTES_MASK,
	...
	/* USB OTG UVLO threshold configuration */
	if (chip->otg_batt_uvlo_thresh!= -EINVAL) {
		for (i = 0; i < ARRAY_SIZE(otg_uvlo_mv); i++) {
			if (otg_uvlo_mv[i] >= chip->otg_batt_uvlo_thresh)
				break;
		}

		if (i == ARRAY_SIZE(otg_uvlo_mv))
			i = i - 1;

		/*0 and 1 is reserved, i starts from 2*/
        i += 2;
		printk("OTG UVLO threshold i = %d \n",i);
		rc = smb1360_masked_write(chip, OTG_UVLO_VOLTAGE,
						OTG_UVLO_THRESH_MASK,
					i << OTG_UVLO_THRESH_SHIFT);
		if (rc)
			pr_err("Couldn't set OTG UVLO threshold, rc = %d\n", rc);
	}
...
static int smb1360_delayed_hw_init(struct smb1360_chip *chip)
{
    ...
    if (!boot_charger_status) {
        rc = smb1360_csir_predicate(chip);
        if (rc < 0)
            pr_err("smb1360 csir version predicated failed\n");

        smb1360_switch_profile(chip);
	...
	pr_info("delayed hw init complete!\n");
	return rc;
}
...
static int smb_parse_dt(struct smb1360_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,otg-batt-uvlo-thresh",
						&chip->otg_batt_uvlo_thresh);
	if (rc < 0)
		chip->otg_batt_uvlo_thresh= -EINVAL;
...
static void smb_capacity_work(struct work_struct *work)
{
	struct smb1360_chip *chip = container_of((struct delayed_work *)work,
					struct smb1360_chip,capacity_work);
	int voltage,curr_val;
	int temp;
	...
	voltage = smb1360_get_prop_voltage_now(chip);
	curr_val = smb1360_get_prop_current_now(chip);
	UI_SOC = smb1360_get_prop_batt_capacity(chip);
	temp = smb1360_get_prop_batt_temp(chip);


	if (stop_charge_expired != timeout_set_flag) {
		smb1360_masked_write(chip, CFG_SFY_TIMER_CTRL_REG,
		 SAFETY_TIME_DISABLE_BIT, (stop_charge_expired ? 0 : SAFETY_TIME_DISABLE_BIT));
		timeout_set_flag = stop_charge_expired;
	}
	if (voltage <= (chip->voltage_empty_mv * 1000)) {
		smb1360_stay_awake(&chip->smb1360_ws,
			WAKEUP_SRC_EMPTY_SOC);
		schedule_delayed_work(&chip->capacity_work, 5*HZ);
		vbat_dead_count++;
		pr_info("bat is becoming dead %d count %d\n",
			voltage, vbat_dead_count);
		if (vbat_dead_count >= BATT_DEAD_COUNT)
			vbat_is_dead = true;
	} else {
		vbat_dead_count = 0;
		vbat_is_dead = false;
		schedule_delayed_work(&chip->capacity_work, 20*HZ);
		smb1360_relax(&chip->smb1360_ws,
			WAKEUP_SRC_EMPTY_SOC);
	}
	pr_info("voltage = %duv, current = %duA, soc = %d, temp = %d\n",
	 voltage, curr_val, UI_SOC, temp);
				...
				const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&chip->capacity_work, smb_capacity_work);
	INIT_DELAYED_WORK(&chip->otg_reconnect_work, smb_otg_reconnect_work);
	...
	schedule_delayed_work(&chip->capacity_work, 2*HZ);
...
static int smb1360_suspend(struct device *dev)
{
	...
	cancel_delayed_work_sync(&chip->capacity_work);
	if(otg_oc == 1)
		cancel_delayed_work_sync(&chip->otg_reconnect_work);
...
static int smb1360_resume(struct device *dev)
{
	...
	schedule_delayed_work(&chip->capacity_work, HZ/2);
...
static void smb1360_shutdown(struct i2c_client *client)
{
	int rc;
	struct smb1360_chip *chip = i2c_get_clientdata(client);
	smb1360_charging_disable(chip, USER, false);//hs force enable charging
	rc = smb1360_otg_disable(chip);
	if (rc)
		pr_err("Couldn't disable OTG mode rc=%d\n", rc);
