...
#ifdef	CONFIG_HISENSE_INTELLIGENT_CHARGE
#include <linux/thermal.h>
#endif /*CONFIG_HISENSE_INTELLIGENT_CHARGE*/
...
#define DEAD_VBAT_UV		3250000
#define DEAD_COUNT			5
#define DEAD_VOLT_CAL_SOC_MS  3000
...
int backup_ocv_soc(struct qpnp_bms_chip *chip, int ocv_uv, int soc)
{
	int rc;
...
EXPORT_SYMBOL_GPL(get_current_time);
...
{
	if (vbat_dead_count > 0)
		return DEAD_VOLT_CAL_SOC_MS;
...
int get_soc_ocv(struct qpnp_bms_chip *chip, int soc)
{
	int rc, batt_temp;
	...
	return interpolate_ocv(chip->batt_data->pc_temp_ocv_lut,
		batt_temp, soc);
}
EXPORT_SYMBOL(get_soc_ocv);

int get_last_soc_ocv(struct qpnp_bms_chip *chip)
{
	return chip->last_ocv_uv;
}
EXPORT_SYMBOL(get_last_soc_ocv);
		...
		if (chip->last_ocv_uv >= chip->ocv_at_100) {
			...
			if (chip->reported_soc_in_use
				...
				chip->batt_psy->set_property(chip->batt_psy,
					POWER_SUPPLY_PROP_STATUS, &ret);
				pr_err("not use reported_soc_in_use or chip->reported_soc != 100,set discharging\n");
			}
...
static int report_vm_bms_soc(struct qpnp_bms_chip *chip)
{
	...
	if (soc != chip->last_soc)
		pr_info("last_soc=%d calculated_soc=%d soc=%d time_since_last_change=%d,chip->last_ocv_uv=%d\n",
			chip->last_soc, chip->calculated_soc, soc,
			time_since_last_change_sec, chip->last_ocv_uv);
...
static void very_low_voltage_check(struct qpnp_bms_chip *chip, int vbat_uv)
{
	...
	if (vbat_uv < DEAD_VBAT_UV) {
		vbat_dead_count++;
		pr_err("bat is becoming dead %d last_ocv_uv %d count %d\n",
			vbat_uv, chip->last_ocv_uv, vbat_dead_count);
		if (vbat_dead_count >= DEAD_COUNT) {
			vbat_is_dead = true;
			if (chip->bms_psy_registered)
				power_supply_changed(&chip->bms_psy);
		}
...
static void calculate_reported_soc(struct qpnp_bms_chip *chip)
{
	...
	if (chip->reported_soc > chip->last_soc) {
		...
		*/
		...
		pr_err("New reported_soc=%d, last_soc is=%d reported_soc_delta=%d\n",
					chip->reported_soc, chip->last_soc,
					chip->reported_soc_delta);
	} else {
		chip->reported_soc_in_use = false;
...
static void monitor_soc_work(struct work_struct *work)
{
	...
	mutex_lock(&chip->last_soc_mutex);
	/* add for case that therm pin is not connected,
	* if vbat is lower than 3.15v for 30s,report dead
	*/
	rc = get_battery_voltage(chip, &vbat_uv);
	if (rc < 0)
		pr_err("Failed to read battery-voltage rc=%d\n", rc);
	...
	if (!is_battery_present(chip)) {
		...
		if (chip->dt.cfg_use_voltage_soc) {
			...
			if (chip->calculated_soc != new_soc) {
				...
				pr_info("batt_temp=%d,vbat_uv=%d,chip->last_ocv_uv=%d\n",
					batt_temp, vbat_uv, chip->last_ocv_uv);
				/*
...
#ifdef	CONFIG_HISENSE_INTELLIGENT_CHARGE
	sensor_get_temp(chip->dt.sensor_id, &chip->pmic_temp);
	pr_err("pmic temp is =%ld\n", chip->pmic_temp);

	if (chip->pmic_temp > 65000) {
		ret.intval = 2;
		chip->batt_psy->set_property(chip->batt_psy,
			POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL, &ret);
		themal_flag = 1;
	} else if (themal_flag && (chip->pmic_temp < 60000)) {
		ret.intval = 0;
		chip->batt_psy->set_property(chip->batt_psy,
			POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL, &ret);
		themal_flag = 0;
...
static int bms_load_hw_defaults(struct qpnp_bms_chip *chip)
{
	...
	if (boot_ftm_mode) {
		pr_err("factory mode, do special config for fifo interrupt\n");
		qpnp_masked_write_base(chip, chip->base + FIFO_LENGTH_REG,
			S1_FIFO_LENGTH_MASK, 2);
		qpnp_masked_write_base(chip, chip->base + FIFO_LENGTH_REG,
			S2_FIFO_LENGTH_MASK, 2 << S2_FIFO_LENGTH_SHIFT);

		qpnp_masked_write_base(chip,
			chip->base + S1_ACC_CNT_REG, ACC_CNT_MASK, 0x04);
		qpnp_masked_write_base(chip,
			chip->base + S2_ACC_CNT_REG, ACC_CNT_MASK, 0x04);
	}
...
static int parse_bms_dt_properties(struct qpnp_bms_chip *chip)
{
	...
	rc = of_property_read_string(chip->spmi->dev.of_node, key,
				&sensor_type);
	if (rc) {
	...
	chip->dt.sensor_id = sensor_get_id((char *)sensor_type);
	if (chip->dt.sensor_id < 0)
		return chip->dt.sensor_id;
#endif /*CONFIG_HISENSE_INTELLIGENT_CHARGE*/
...
static int qpnp_vm_bms_probe(struct spmi_device *spmi)
{
	...
	/* add for recovery soc calc,init voltage_soc */
	chip->prev_voltage_based_soc = get_prop_bms_capacity(chip);

	pr_info("probe success: soc=%d vbatt=%d ocv=%d warm_reset=%d\n",
					chip->prev_voltage_based_soc, vbatt,
