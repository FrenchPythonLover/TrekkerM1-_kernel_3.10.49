...
#define USB_PTH_STS_MASK			LBC_MASK(7, 6)
#define USB_PTH_STS_OVP				0x40
...
extern int msm_proc_comm_is_meid_null_flag;//hs
...
struct qpnp_lbc_chip {
	...
	/* use for after charger remove, the soc stay in 100 for a while */
	bool				ui_state;
	unsigned long		ui_start_sec;
	unsigned int        last_report_soc;
	...
	#if defined(CONFIG_HISENSE_CHG_AICL)
	struct delayed_work		aicl_work;
	#endif /* CONFIG_HISENSE_CHG_AICL */
	/* parallel-chg params */
...
static int stop_charge_expired;
static int charger_ovp;
module_param(stop_charge_expired, int, 0644);
static int qpnp_lbc_chg_failed_clr(void *_chip);
...
static int qpnp_lbc_is_usb_chg_plugged_in(struct qpnp_lbc_chip *chip)
{
	...
	#if defined(CONFIG_TOUCHSCREEN_FT5X06_FH)
	usb_flag = (usbin_valid_rt_sts & USB_IN_VALID_MASK) ? 1 : 0;
	#endif
...
extern struct qpnp_bms_chip *the_chip;
#define DEFAULT_CAPACITY	50
extern int get_current_time(unsigned long *now_tm_sec);
extern int backup_ocv_soc(struct qpnp_bms_chip *chip, int ocv_uv, int soc);
extern int get_soc_ocv(struct qpnp_bms_chip *chip, int soc);
extern int get_last_soc_ocv(struct qpnp_bms_chip *chip);
static int get_prop_capacity(struct qpnp_lbc_chip *chip)
{
	...
	if (chip->bms_psy) {
		...
		if ((ret.intval < chip->cfg_soc_resume_limit)
			&& chip->chg_done) {
			chip->chg_done = false;
			power_supply_changed(&chip->batt_psy);
			pr_err("reset chg_done flag, bms soc is %d\n",
				ret.intval);
		}

		/* when charger remove, if last reported soc is 100,
		* bms calculated soc is less than 100,
		* keep soc 100 for 1 minute
		*/
		if ((chip->last_report_soc == 100) && (ret.intval < 100)
			&& (!charger_in) && (!chip->ui_state)) {
			pr_err("ui state started, bms soc is %d\n", ret.intval);
			chip->ui_state = true;
			get_current_time(&chip->ui_start_sec);
		}
		/* stop recharging */
		if ((ret.intval == 100) && charger_in) {
			pr_err("recharge fininshed, stop charging\n");
		...
		if (battery_status != POWER_SUPPLY_STATUS_CHARGING
			...
			pr_err("resuming charging at %d%% soc\n",
					ret.intval);
		...
		/* to make sure soc stays in 100 during recharging */
		if (charger_in && chip->chg_done)
			ret.intval = 100;
		...
		if (chip->ui_state) {
			...
			delta_time = now_tm_sec - chip->ui_start_sec;
			if (delta_time > 60) {
				soc = ret.intval;
				pr_err("ui state ended bms soc is %d\n",
					ret.intval);
				chip->ui_state = false;
			} else {
				soc = 100;
				pr_err("ui state report soc to 100, bms soc is %d\n",
					ret.intval);
			}
		}
		/* to make sure the 100 match with FULL status */
		if (charger_in && soc == 100)
			chip->chg_done = true;

		/* backup soc and ocv,if soc is 100,
		* backup bigger value to ensure reboot soc is 100
		*/
		if (soc == 100)
			ocv_uv = get_soc_ocv(the_chip, soc) * 1000;
		else
			ocv_uv = get_last_soc_ocv(the_chip);

		pr_debug("backup soc %d,ocv %d\n", soc, ocv_uv);
		backup_ocv_soc(the_chip, ocv_uv, soc);
...
#if defined(CONFIG_HISENSE_CHG_AICL)
static void qpnp_lbc_aicl_set_current(struct qpnp_lbc_chip *chip,
						int aicl_current)
{
	unsigned int chg_current = aicl_current;
	...
	if (qpnp_lbc_is_usb_chg_plugged_in(chip)) {
		...
		if (current_ma <= 2 && !chip->cfg_use_fake_battery
			...
			if (chip->bat_is_cool
				...
				schedule_delayed_work(&chip->aicl_work, 0);
			}
#else /* CONFIG_HISENSE_CHG_AICL */
			qpnp_lbc_set_appropriate_current(chip);
			qpnp_lbc_charger_enable(chip, CURRENT, 1);
#endif /* CONFIG_HISENSE_CHG_AICL */
		}
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if (val->intval == POWER_SUPPLY_STATUS_FULL &&
			...
			//Let phone boot up without vbat for factory test of PCBA project.
			if( !get_prop_batt_present(chip)
					&& (msm_proc_comm_is_meid_null_flag == 1 )) {//hs
					pr_info("battery absent and imei is null,don't stop charge\n");
			}
			else {
			pr_err("full,stop charge\n");
			/* Disable charging */
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		chip->last_report_soc = val->intval;
		break;
					...
					int enable)
{
	...
	pr_info("charging=%d ichg_now=%d max_chg_current=%d\n",
...
#ifdef	CONFIG_HISENSE_SCALE_BOARD_THERM
	POWER_SUPPLY_PROP_TEMP,
#endif /*CONFIG_HISENSE_SCALE_BOARD_THERM*/
};
...
static int qpnp_charger_read_dt_props(struct qpnp_lbc_chip *chip)
{
	...
	/*Get the stop-charge-expired property */
	stop_charge_expired =
			of_property_read_bool(chip->spmi->dev.of_node,
					"qcom,stop-charge-expired");
	...
	pr_err("chip->cfg_disable_vbatdet_based_recharge is %d\n",
		chip->cfg_disable_vbatdet_based_recharge);
...
static irqreturn_t qpnp_lbc_coarse_det_usb_irq_handler(int irq, void *_chip)
{
	...
	pr_err("usb_present is %d\n", usb_present);
	if ((usb_present == 0) && charger_ovp) {
...
#if defined(CONFIG_HISENSE_CHG_AICL)
#define USB_VALID_VOLTAGE	4300000
static int aicl_count;
static int aicl_revert_count;
#endif /* CONFIG_HISENSE_CHG_AICL */
...
static irqreturn_t qpnp_lbc_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	unsigned long flags;
	u8 usb_chg_sts;
	chip->tchg_min_extended = false;
	qpnp_lbc_chg_failed_clr(chip);
	usb_present = qpnp_lbc_is_usb_chg_plugged_in(chip);
	pr_err("usbin-valid triggered: %d\n", usb_present);

	qpnp_lbc_read(chip, chip->usb_chgpth_base + USB_PTH_STS_REG,
				&usb_chg_sts, 1);
	if ((usb_chg_sts & USB_PTH_STS_MASK) == USB_PTH_STS_OVP) {
		charger_ovp  = 1;
		pr_info("qpnp_lbc_usbin_valid_irq_handler, charger is ovp!\n");
	} else
		charger_ovp = 0;
	...
	if (chip->usb_present ^ usb_present) {
		...
		if (!usb_present) {
			...
			aicl_count = 0;
			aicl_revert_count = 0;
			cancel_delayed_work(&chip->aicl_work);
			#endif /* CONFIG_HISENSE_CHG_AICL */
		} else {
			/*
		...
		pr_err("Updating usb_psy PRESENT property\n");
		if (charger_ovp)
			pr_err("charger is ovp,keep usb online\n");
		else
...
static void qpnp_ctp_work(struct work_struct *work)
{
	struct qpnp_lbc_chip *chip = container_of(work,
					struct qpnp_lbc_chip,ctp_work);
...
#if defined(CONFIG_HISENSE_CHG_AICL)
static void qpnp_aicl_work(struct work_struct *work)
{
	struct qpnp_lbc_chip *chip = container_of(work,
					struct qpnp_lbc_chip, aicl_work.work);

	static int setting_current = 180;
	struct qpnp_vadc_result usbin;

	if (!chip->usb_present)
		return;

	if (chip->usb_psy_ma <= 500) {
		pr_err("no need to do the aicl because it is not DCP\n");
		aicl_count = 0;
		aicl_revert_count = 0;
		qpnp_lbc_set_appropriate_current(chip);
		qpnp_lbc_charger_enable(chip, CURRENT, 1);
	...
	if (aicl_count == 0) {
		pr_err("aicl start work from begining\n");
		setting_current = 180;
		aicl_count++;
		qpnp_lbc_aicl_set_current(chip, setting_current);
		qpnp_lbc_charger_enable(chip, CURRENT, 1);
		qpnp_vadc_read(chip->vadc_dev, USBIN, &usbin);
		if (usbin.physical < USB_VALID_VOLTAGE) {
			pr_err("plug in voltage is to low, abort aicl\n");
			return;
		}
		schedule_delayed_work(&chip->aicl_work, msecs_to_jiffies(1000));
		return;
	} else {
		qpnp_vadc_read(chip->vadc_dev, USBIN, &usbin);
		pr_err("usbin voltage is %lld\n", usbin.physical);
		if ((usbin.physical > USB_VALID_VOLTAGE)
			&& (aicl_revert_count > 0)) {
			aicl_revert_count = 0;
			aicl_count = 0;
			pr_err("aicl work finished from revert current\n");
			return;
		}
		if (usbin.physical < USB_VALID_VOLTAGE) {
			pr_err("aicl current set to %d to increase the vbus voltage\n",
				setting_current);
			setting_current = setting_current - 90;
			aicl_count++;
			aicl_revert_count++;
			qpnp_lbc_aicl_set_current(chip, setting_current);
			qpnp_lbc_charger_enable(chip, CURRENT, 1);
			...
			/* 90ma is the step to adjust the current in register */
			if (setting_current <= (chip->usb_psy_ma-90)) {
				setting_current = setting_current + 90;
				aicl_count++;
				pr_err("aicl current set to %d\n",
					setting_current);
				qpnp_lbc_aicl_set_current(chip,
					setting_current);
				qpnp_lbc_charger_enable(chip, CURRENT, 1);
				schedule_delayed_work(&chip->aicl_work,
					msecs_to_jiffies(1000));
			} else {
				pr_err("aicl work finished, final current is %d\n",
					chip->usb_psy_ma);
			}
		}
	}
...
static irqreturn_t qpnp_lbc_batt_pres_irq_handler(int irq, void *_chip)
{
	...
	if (chip->batt_present ^ batt_present) {
		...
		if(msm_proc_comm_is_meid_null_flag == 0) {
...
/* chgr stop if don't clr failed bit */
static int qpnp_lbc_chg_failed_clr(void *_chip)
{
	struct qpnp_lbc_chip *chip = _chip;
	int rc;
...
static irqreturn_t qpnp_lbc_chg_failed_irq_handler(int irq, void *_chip)
{
	...
	/*charge_expired,then stop,else only clr*/
	if (stop_charge_expired) {
		if (chip->tchg_min_extended)
			pr_err("chg expired,stop\n");
		else {
			pr_err("chg expired,extended\n");
			rc = qpnp_lbc_chg_failed_clr(chip);
			chip->tchg_min_extended = true;
		}
	} else
		rc = qpnp_lbc_chg_failed_clr(chip);
...
static irqreturn_t qpnp_lbc_fastchg_irq_handler(int irq, void *_chip)
{
	...
	pr_err("FAST_CHG IRQ triggered, fastchg_on: %d\n", fastchg_on);
...
static irqreturn_t qpnp_lbc_chg_done_irq_handler(int irq, void *_chip)
{
	...
	pr_err("charging done triggered\n");
...
static int qpnp_lbc_parallel_charger_init(struct qpnp_lbc_chip *chip)
{
	...
	/* Enable THM and disable BID based BPD */
...
static int qpnp_lbc_parallel_probe(struct spmi_device *spmi)
{
	...
	if (IS_ERR(chip->vadc_dev)) {
		...
		if (rc == -EPROBE_DEFER) {
			pr_err("qpnp vadc not found - defer rc=%d\n", rc);
			return rc;
		} else
			pr_err("qpnp vadc property missing, rc=%d\n", rc);
	}
	...
	pr_info("LBC (parallel) registered successfully, batt_present = %d!\n",
		chip->batt_present);
...
#if defined(CONFIG_TOUCHSCREEN_FT5X06_FH)
	INIT_WORK(&chip->ctp_work,qpnp_ctp_work);
	if(chip->usb_present)
		ctp_work_with_ac_usb_plugin(1);
#endif

#if defined(CONFIG_HISENSE_CHG_AICL)
	INIT_DELAYED_WORK(&chip->aicl_work, qpnp_aicl_work);
#endif /* CONFIG_HISENSE_CHG_AICL */
