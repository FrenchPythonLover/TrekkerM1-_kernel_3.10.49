...
static void msm_chg_detect_work(struct work_struct *w)
{
	...
	#ifdef CONFIG_HISENSE_USB_FEATURE
	if (!test_bit(ID, &motg->inputs)) {
		pr_info("ID is grounded,escape chg detection work,just return\n");
		return;
	}
	#endif /* CONFIG_HISENSE_USB_FEATURE */
...
#define RECHECK_DISABLE 0
#define RECHECK_ENABLE 1
#define RECHECKED 2
static void msm_floated_charger_check_timer_func(unsigned long data)
{
	struct msm_otg *motg = (struct msm_otg *) data;

	if(RECHECK_ENABLE == motg->floated_charger_recheck){
		pr_info("@@@@floated charger recheck\n");
		motg->floated_charger_recheck = RECHECKED;
		motg->chg_type = USB_INVALID_CHARGER;
		motg->chg_state = USB_CHG_STATE_UNDEFINED;
		queue_work(motg->otg_wq, &motg->sm_work);
	}
		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_FLOATED_CHARGER:
#ifndef CONFIG_HISENSE_USB_FEATURE
					msm_otg_notify_charger(motg,
							IDEV_CHG_MAX);
					otg->phy->state =
						OTG_STATE_B_CHARGER;
					...
					pm_runtime_put_noidle(otg->phy->dev);
					pm_runtime_suspend(otg->phy->dev);
#else /* CONFIG_HISENSE_USB_FEATURE */
					work = 0;
					if(RECHECK_DISABLE == motg->floated_charger_recheck) {
						msm_otg_notify_charger(motg, IDEV_CHG_MIN);
						motg->floated_charger_recheck = RECHECK_ENABLE;
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		#ifdef CONFIG_HISENSE_USB_FEATURE
		motg->floated_charger_recheck = RECHECK_DISABLE;
		#endif /* CONFIG_HISENSE_USB_FEATURE */
