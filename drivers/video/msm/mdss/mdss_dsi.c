...
/* add by hmct */
static char mdss_panel_esd[ESD_CHECK_PANEL_SIZE];
/* add by hmct for lcd_id */
static char mdss_panel_lcd_id[32];
...
*/
/* add for enp\enn by hmct */
	if (gpio_is_valid(ctrl_pdata->lcdenp_gpio))
		gpio_direction_output((ctrl_pdata->lcdenp_gpio), 0);
	usleep(pdata->panel_info.enp_delay_us);
	if (gpio_is_valid(ctrl_pdata->lcdenn_gpio))
		gpio_direction_output((ctrl_pdata->lcdenn_gpio), 0);
...
/*
* Hmct modify
*  lcd ENP and ENN GPIO contrl for external +/-5V power.
*/
	if (mdss_dsi_pinctrl_set_state(ctrl_pdata, true))
		pr_debug("lcd gpio enable: pinctrl not enabled\n");

	if (gpio_is_valid(ctrl_pdata->lcdenp_gpio))
		gpio_direction_output((ctrl_pdata->lcdenp_gpio), 1);

	usleep(pdata->panel_info.enp_delay_us);

	if (gpio_is_valid(ctrl_pdata->lcdenn_gpio))
		gpio_direction_output((ctrl_pdata->lcdenn_gpio), 1);
	/*
	 * If continuous splash screen feature is enabled,
	 * do not reset panel again
	 *modify by Hmct
	 */
...
/* hmct modification.parse backlight node */
static struct device_node *mdss_dsi_find_backlight_of_node(struct platform_device *pdev)
{
	struct device_node *dsi_bl_node = NULL, *mdss_node = NULL;
	...
	dsi_bl_node = of_find_node_by_name(mdss_node,
					    "qcom,mdss_dsi_bl_cfg");
	if (!dsi_bl_node) {
		pr_err("%s: invalid backlight node!\n",
		       __func__);
		return NULL;
	}
	return dsi_bl_node;
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct device_node *dsi_pan_node = NULL;
	/* hmct modification.backlight node */
	struct device_node *dsi_bl_node = NULL;
	...
	/* add by hmct for lcd_id */
	ctrl_pdata->panel_data.panel_info.panel_lcd_id = mdss_panel_lcd_id;
	...
	/* hmct modification. add backlight node */
	dsi_bl_node = mdss_dsi_find_backlight_of_node(pdev);
	if (!dsi_bl_node) {
		pr_err("%s: can't find backlight node\n", __func__);
		of_node_put(dsi_bl_node);
	}

	/* hmct modification.backlight init */
	rc = mdss_dsi_backlight_init(dsi_bl_node, ctrl_pdata);
	if (rc) {
		pr_err("%s: dsi backlight init failed\n", __func__);
		of_node_put(dsi_bl_node);	/* goto error_bl_node; */
	}
...
static int mdss_dsi_phy_timing_config(struct mdss_panel_info *pinfo)
{
	...
	printk("dsi phy:%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,0, %x,%x,clk_rate=%d\n",
...
module_param_string(esd_check, mdss_panel_esd, ESD_CHECK_PANEL_SIZE, 0);
