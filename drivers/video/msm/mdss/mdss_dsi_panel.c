...
#include <linux/err.h>
/* Hmct add for getting LCD Hardware info */
...
/* Hmct add for bl log */
	...
	if ((last_level && bl_level) == 0) {
		printk("mdss_dsi_panel_bl_ctrl: set lcd backlight %d to %d\n", last_level, bl_level);
		last_level = bl_level;
	}
...
/* hmct add lcd init log */
	printk("###Lcd panel init start###\n");
	if (ctrl->on_cmds.cmd_cnt)
		mdss_dsi_panel_cmds_send(ctrl, &ctrl->on_cmds);
	printk("###lcd panel init completed###\n");
end:
...
static void mdss_dsi_parse_esd_check_params(struct device_node *np,
		u32 esd_params[MDSS_DSI_ESD_CHECK_PARAMS_NUM],
		u32 *esd_params_len, const char *name)
{
	int num = 0, i;
	int rc;
	struct property *data;
...
*/
	...
	/* hmct add
	* backlight delay open when wake up
	*/
	rc = of_property_read_u32(np, "qcom,mdss-dsi-bl-delay-ms", &tmp);
	pinfo->bl_delay_ms = (!rc ? tmp : 0);
...
*/
	data = of_get_property(np, "qcom,mdss-dsi-panel-status-command-state", NULL);
	if (data && !strcmp(data, "dsi_hs_mode"))
		ctrl_pdata->status_cmds.link_state = DSI_HS_MODE;
	else
		ctrl_pdata->status_cmds.link_state = DSI_LP_MODE;
...
/* add delay between enp and enn,Hmct add */
	rc = of_property_read_u32(np, "qcom,mdss-dsi-enp-delay-us", &tmp);
	pinfo->enp_delay_us = (!rc ? tmp : 0);
	...
	bool cmd_cfg_cont_splash)
{
	...
	if (!panel_name) {
		...
		printk("%s: dsi panel mdss panel lcd_id: %s\n", __func__, pinfo->panel_lcd_id);
/* Hmct add for lcm info */
		printk("lcd panel name: %s\n", panel_name);
		productinfo_register(PRODUCTINFO_LCD_ID, panel_name, pinfo->panel_lcd_id);
	}
...
/* hmct modification. backlight init func */
int mdss_dsi_backlight_init(struct device_node *node,
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	u32 tmp;
	int rc = 0;
	...
	ctrl_pdata->bklt_ctrl = UNKNOWN_CTRL;
	data = of_get_property(node, "qcom,mdss-dsi-bl-pmic-control-type", NULL);
	if (data) {
		if (!strncmp(data, "bl_ctrl_wled", 12)) {
			led_trigger_register_simple("bkl-trigger",
				&bl_led_trigger);
	...
	rc = of_property_read_u32(node, "qcom,mdss-brightness-max-level", &tmp);
	pinfo->brightness_max = (!rc ? tmp : MDSS_MAX_BL_BRIGHTNESS);
	rc = of_property_read_u32(node, "qcom,mdss-dsi-bl-min-level", &tmp);
	pinfo->bl_min = (!rc ? tmp : 0);
	rc = of_property_read_u32(node, "qcom,mdss-dsi-bl-max-level", &tmp);
	pinfo->bl_max = (!rc ? tmp : 255);
	ctrl_pdata->bklt_max = pinfo->bl_max;
