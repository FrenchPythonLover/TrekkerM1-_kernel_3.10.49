...
#define BOOT_TIME_SECS  30	/*assume boot up time is 30s*/
...
/*SCM CALL Failed sometimes,HS add a WAR to reboot phone in this case*/
static void scm_retry_failed_war(void)
{
	u64 time_now = 0;
	time_now  = local_clock();
	do_div(time_now, 1000000000);
	/*reset phone if tz error after retry 60 times*/
	if (time_now < BOOT_TIME_SECS) {
		pr_err("SCM Retry Failed,Reset Phone\n");
		if (arm_pm_restart)
