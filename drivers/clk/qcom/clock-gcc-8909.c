...
/********hisense added for PWM flash control ---start********/
#define F_DUTY_CYCLE(f, s, div, m, n, level) \
	{ \
		.freq_hz = (f), \
		.src_clk = &s##_clk_src.c, \
		.m_val = (m), \
...
/********hisense added for PWM flash control ---end********/
...
static struct clk_freq_tbl ftbl_gcc_camss_gp0_1_clk[] = {
	F_DUTY_CYCLE( 192000,	      xo,	1,	1,    100,		37),//hisense added for PWM flash control 
	F( 100000000,	gpll0,	8,	0,	0),
	F( 200000000,	gpll0,	4,	0,	0),
	F_END
};
...
void  set_gcc_camss_gp0_1_clk_duty(int duty)
{
	int duty_cycle;
	int n = 100;
	duty_cycle = ~(2 * (duty * n / 100));
	ftbl_gcc_camss_gp0_1_clk[0].d_val = duty_cycle;
}
EXPORT_SYMBOL(set_gcc_camss_gp0_1_clk_duty);
