...
#define SELFIE_IMPEDANCE_VALUE 20000
...
#ifdef CONFIG_HIS_AUDIO_EXT_SPK_PA
	if(!is_session_playback) //when rx flow has data,do not power of PA
#endif
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	/*
	* if selfie device is detected as MBHC_PLUG_TYPE_HEAD in function wcd_mbhc_detect_plug_type,
	* reset it to MBHC_PLUG_TYPE_NONE to avoid the second time,selfie can not be detected
	*/
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET) {
		mbhc->current_plug = MBHC_PLUG_TYPE_NONE;
	}
	...
	if (!rc) {
		...
		if (!btn_result && !hs_comp_res)
			...
			/*
			* calc impedance to check whether the device is selfie add by hisense
			*/
			WCD_MBHC_RSC_LOCK(mbhc);
			if (mbhc->impedance_detect &&
			mbhc->mbhc_cb->compute_impedance ) {
			...
			WCD_MBHC_RSC_UNLOCK(mbhc);
			if(mbhc->zl > SELFIE_IMPEDANCE_VALUE || mbhc->zr > SELFIE_IMPEDANCE_VALUE)
				plug_type = MBHC_PLUG_TYPE_HEADSET;
			else
	...
	pr_debug("%s: Valid plug found, plug type is %d\n",
			 __func__, plug_type);
	/*
	*if plug type is headset , jump correct plug switch,to avoid noise
...
static void wcd_btn_lpress_fn(struct work_struct *work)
{
	...
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET) {
		pr_info("%s: Reporting long button press event, btn_result: %d\n",
			 __func__, btn_result);/*Hisense modified*/
