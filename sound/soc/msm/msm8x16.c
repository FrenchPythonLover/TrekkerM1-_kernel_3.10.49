...
static struct wcd_mbhc_config mbhc_cfg = {
	...
	.key_code[6] = 0,
	.key_code[7] = 0,
	.linein_th = 0,/*Hisense modify this value 5000->0,*/
};
...
#ifdef CONFIG_HIS_AUDIO_EXT_SPK_PA
struct ext_spk_headset_pinctrl_info {
	struct pinctrl *pinctrl;
	struct pinctrl_state *ext_spk_pa_act;
	struct pinctrl_state *ext_spk_pa_sus;
	struct pinctrl_state *headset_switch_act;
	struct pinctrl_state *headset_switch_sus;
};

static struct ext_spk_headset_pinctrl_info ext_spk_pinctrl_info;
#endif
...
static int msm8x16_headset_switch_init(void)
{
	...
	if (headset_switch_gpio >= 0) {
		...
		pinctrl = devm_pinctrl_get(&spdev->dev);
		if (IS_ERR(pinctrl)) {
			pr_err("%s: Unable to get pinctrl handle\n", __func__);
			return -EINVAL;
		}
		ext_spk_pinctrl_info.pinctrl = pinctrl;
		/* get pinctrl handle for headset switch pin*/
		ext_spk_pinctrl_info.headset_switch_sus = pinctrl_lookup_state(pinctrl,
							"headset_switch_sus");
		if (IS_ERR(ext_spk_pinctrl_info.headset_switch_sus)) {
			pr_err("%s: Unable to get pinctrl disable handle\n",
								  __func__);
			return -EINVAL;
		}

		ext_spk_pinctrl_info.headset_switch_act = pinctrl_lookup_state(pinctrl,
							"headset_switch_act");
		if (IS_ERR(ext_spk_pinctrl_info.headset_switch_act)) {
			pr_err("%s: Unable to get pinctrl active handle\n",
								 __func__);
			return -EINVAL;
		}

		ret = pinctrl_select_state(ext_spk_pinctrl_info.pinctrl,
					ext_spk_pinctrl_info.headset_switch_act);
		if (ret < 0) {
...
static int msm8x16_ext_spk_pa_init(void)
{
	...
	if (ext_spk_pa_gpio >= 0) {
		...
		ext_spk_pinctrl_info.pinctrl = pinctrl;
		/* get pinctrl handle for ext spk pin*/
		ext_spk_pinctrl_info.ext_spk_pa_sus = pinctrl_lookup_state(pinctrl,
							"ext_spk_pa_sus");
		if (IS_ERR(ext_spk_pinctrl_info.ext_spk_pa_sus)) {
			pr_err("%s: Unable to get pinctrl disable handle\n",
								  __func__);
			return -EINVAL;
		}

		ext_spk_pinctrl_info.ext_spk_pa_act = pinctrl_lookup_state(pinctrl,
							"ext_spk_pa_act");
		if (IS_ERR(ext_spk_pinctrl_info.ext_spk_pa_act)) {
			pr_err("%s: Unable to get pinctrl active handle\n",
								 __func__);
			return -EINVAL;
		}
		...
		ret = pinctrl_select_state(ext_spk_pinctrl_info.pinctrl,
					ext_spk_pinctrl_info.ext_spk_pa_act);
		if (ret < 0) {
			pr_err("failed to configure the gpio\n");
			return ret;
		}
